...
=item void zco_unregister_callback(void)

This function simply unregisters the currently registered callback function
for ZCO destruction.

=item SdrObject zco_create_file_ref(Sdr sdr, char *pathName,
char *cleanupScript, ZcoAcct acct)
...
=item SdrObject zco_revise_file_ref(Sdr sdr, SdrObject fileRef, char *pathName,
char *cleanupScript)
...
=item char *zco_file_ref_path(Sdr sdr, SdrObject fileRef, char *buffer,
int buflen)
...
=item int zco_file_ref_xmit_eof(Sdr sdr, SdrObject fileRef)
...
=item void zco_destroy_file_ref(Sdr sdr, SdrObject fileRef)
...
=item SdrObject zco_create_bulk_ref(Sdr sdr, unsigned long item, vast length,
ZcoAcct acct)
...
=item void zco_destroy_bulk_ref(Sdr sdr, SdrObject bulkRef)
...
=item SdrObject zco_create_obj_ref(Sdr sdr, SdrObject object, vast length,
ZcoAcct acct)
...
=item void zco_destroy_obj_ref(Sdr sdr, SdrObject objRef)
...
=item double zco_get_bulk_occupancy(Sdr sdr, ZcoAcct acct)

Returns the total number of mass storage space bytes occupied by ZCOs (inbound
or outbound) created in this Sdr.

=item void zco_set_max_bulk_occupancy(Sdr sdr, double occupancy, ZcoAcct acct)

Declares the total number of mass storage space bytes that may be occupied by
ZCOs (inbound or outbound) created in this Sdr.

=item double zco_get_max_bulk_occupancy(Sdr sdr, ZcoAcct acct)
...
=item double zco_get_heap_occupancy(Sdr sdr, ZcoAcct acct)

Returns the total number of SDR heap space bytes occupied by ZCOs (inbound or
outbound) created in this Sdr.

=item void zco_set_max_heap_occupancy(Sdr sdr, double occupancy, ZcoAcct acct)

Declares the total number of SDR heap space bytes that may be occupied by
ZCOs (inbound or outbound) created in this Sdr.

=item double zco_get_max_heap_occupancy(Sdr sdr, ZcoAcct acct)
...
=item int zco_get_aggregate_length(Sdr sdr, SdrObject location, vast offset,
...
=item SdrObject zco_create(Sdr sdr, ZcoMedium firstExtentSourceMedium,
SdrObject firstExtentLocation, vast firstExtentOffset, vast firstExtentLength,
ZcoAcct acct)
...
of the new ZCO; returns ((SdrObject) -1) on any error.

=item int zco_append_extent(Sdr sdr, SdrObject zco, ZcoMedium sourceMedium,
...
=item int zco_prepend_header(Sdr sdr, SdrObject zco, char *header, vast length)

=item int zco_append_trailer(Sdr sdr, SdrObject zco, char *trailer, vast length)

=item void zco_discard_first_header(Sdr sdr, SdrObject zco)

=item void zco_discard_last_trailer(Sdr sdr, SdrObject zco)
...
=item SdrObject zco_header_text(Sdr sdr, SdrObject zco, int skip, vast *length)
...
=item SdrObject zco_trailer_text(Sdr sdr, SdrObject zco, int skip, vast *length)
...
=item void zco_destroy(Sdr sdr, SdrObject zco)
...
=item void zco_bond(Sdr sdr, SdrObject zco)
...
=item int zco_revise(Sdr sdr, SdrObject zco, vast offset, char *buffer,
vast length)

Writes the contents of I<buffer>, for length I<length>, into I<zco> at offset
I<offset>.  Returns 0 on success, -1 on any error.

=item SdrObject zco_clone(Sdr sdr, SdrObject zco, vast offset, vast length)
...
SDR location of the new ZCO on success, (SdrObject) -1 on any error.

=item vast zco_clone_source_data(Sdr sdr, SdrObject toZco, SdrObject fromZco,
...
=item vast zco_length(Sdr sdr, SdrObject zco)
...
=item vast zco_source_data_length(Sdr sdr, SdrObject zco)
...
=item ZcoAcct zco_acct(Sdr sdr, SdrObject zco)

Returns an indicator as to whether I<zco> is inbound or outbound.

=item void zco_start_transmitting(SdrObject zco, ZcoReader *reader)
...
=item void zco_start_receiving(SdrObject zco, ZcoReader *reader)
...
=item void zco_delimit_source(Sdr sdr, SdrObject zco, vast offset, vast length)
...
=item void zco_strip(Sdr sdr, SdrObject zco)
