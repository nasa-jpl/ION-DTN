...
=item SdrObject sdr_list_create(Sdr sdr)
...
=item void sdr_list_destroy(Sdr sdr, SdrObject list, SdrListDeleteFn fn,
void *arg)

Destroys a list, freeing all elements of list.  If I<fn> is non-NULL,
that function is called once for each freed element;
when called, I<fn> is passed the SdrAddress that is the element's data and
...
=item size_t sdr_list_length(Sdr sdr, SdrObject list)

Returns the number of elements in the list, or -1 on any error.

=item void sdr_list_user_data_set(Sdr sdr, SdrObject list,
SdrAddress userData)

Sets the "user data" word of I<list> to I<userData>.  Note that
I<userData> is nominally an SdrAddress but can in fact be any value
...
=item SdrAddress sdr_list_user_data(Sdr sdr, SdrObject list)

Returns the value of the "user data" word of I<list>, or zero on any error.

=item SdrObject sdr_list_insert(Sdr sdr, SdrObject list, SdrAddress data,
...
=item SdrObject sdr_list_insert_first(Sdr sdr, SdrObject list, SdrAddress data)

=item SdrObject sdr_list_insert_last(Sdr sdr, SdrObject list, SdrAddress data)
...
=item SdrObject sdr_list_insert_before(Sdr sdr, SdrObject elt, SdrAddress data)

=item SdrObject sdr_list_insert_after(Sdr sdr, SdrObject elt, SdrAddress data)
...
=item void sdr_list_delete(Sdr sdr, SdrObject elt, SdrListDeleteFn fn, void *arg)

Delete I<elt> from the list it is in.
If I<fn> is non-NULL, that function will be called upon deletion of
I<elt>; when called, that function is passed the SdrAddress that is the list
element's data value and the I<arg> pointer passed to sdr_list_delete().

=item SdrObject sdr_list_first(Sdr sdr, SdrObject list)

=item SdrObject sdr_list_last(Sdr sdr, SdrObject list)

Returns the address of the first/last element of I<list>, or zero on
any error.

=item SdrObject sdr_list_next(Sdr sdr, SdrObject elt)

=item SdrObject sdr_list_prev(Sdr sdr, SdrObject elt)

Returns the address of the element following/preceding I<elt>
in that element's list, or zero on any error.

=item SdrObject sdr_list_search(Sdr sdr, SdrObject elt, int reverse,
SdrListCompareFn fn, void *dataBuffer)
...
element); each time I<compare> is called it is passed the SdrAddress that is
...
located whose data value is equal to ((SdrAddress) I<dataBuffer>).  Returns
the address of the matching element if one is found, 0 otherwise.

=item SdrObject sdr_list_list(Sdr sdr, SdrObject elt)

Returns the address of the list to which I<elt> belongs,
or 0 on any error.

=item SdrAddress sdr_list_data(Sdr sdr, SdrObject elt)

Returns the SdrAddress that is the data value of I<elt>, or 0 on any error.

=item SdrAddress sdr_list_data_set(Sdr sdr, SdrObject elt, SdrAddress data)
...
list element (I<eltData>, nominally the SdrAddress of an item in the SDR's
...
When provided, this function is automatically called by the sdrlist function
being invoked; when the function is called it is passed the content of a
list element (I<eltData>, nominally the SdrAddress of an item in the SDR's heap
