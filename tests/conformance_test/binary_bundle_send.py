...
def bundle_send(str_bundle: str, port: int) -> None:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sends bundle(s) that have been passed as hexadecimal strings via command line or a file.")
    parser.add_argument("-p", "--port", help="UDP port to send bundle, default: 3113", default=3113)
    parser.add_argument("-b", "--bundle", help="Hexadecimal string representation of a full bundle", action="append")
    parser.add_argument("-f", "--file", help="Path to file with a hexadecimal string representation of a full bundle on each line.")
    ...
    if args.file:
        ...
        bundle_list = [item.strip() for item in init_bundle_list]
    elif args.bundle:
