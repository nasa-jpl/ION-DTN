...
int	main(int argc, char **argv)
	...
	if(argc>2)
		...
		if(access(myfilename, F_OK) == 0)
			{
			isprintf(progressText, sizeof progressText, "[i] file '%s' exists. bprecvfile will append", myfilename);
			writeMemo(progressText);
			} else
			{
			isprintf(progressText, sizeof progressText, "[i] bprecvfile will create file '%s'", myfilename);
			writeMemo(progressText);
			}
