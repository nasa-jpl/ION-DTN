...
=item int sdr_hash_insert(Sdr sdr, Object hash, char *key, Address value)

Inserts an entry into the hash table identified by I<hash>.  Returns zero on
success, -1 on any error.

=item int sdr_hash_retrieve(Sdr sdr, Object hash, char *key, Address *value)

Searches for the value associated with I<key> in this hash table, storing it in
I<value> if found.  Returns 1 if the entry matching I<key> was found, zero if
...
=item int sdr_hash_remove(Sdr sdr, Object hash, char *key)

Searches for the hash table entry matching I<key> in this hash table,
deleting it if it is found.  Returns 1 if the entry matching I<key> was
found, zero if no such entry exists, -1 on any other failure.
