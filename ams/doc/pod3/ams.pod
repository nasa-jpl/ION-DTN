...
the "ipn" endpoint identifier scheme with B<node identifier> set to the fully
qualified node number that is equal to the AMS B<continuum number> and
B<service number> set to the AMS B<venture number>.  NOTE that AMS continuum
numbers are currently expressed as signed 32-bit integers; FQNNs may be
truncated when used as continuum numbers.
...
Selection of the configuration file format is a configure-time decision,
implemented by setting (or not setting) the --with-expat configure option.
...
named "mib.amsrc" if AMS was built using --without-expat (default),
"amsmib.xml" otherwise.
...
Reverses the operation of ams_register(), destroying I<module>.  Returns
0 on success, -1 on any error.

=item int ams_invite(AmsModule module, int roleNbr, short continuumNbr,
int unitNbr, short subjectNbr, int priority, unsigned char flowLabel,
...
identified by I<continuumNbr> (where 0 indicates "all continua"), and
...
=item int ams_disinvite(AmsModule module, int roleNbr, short continuumNbr,
...
=item int ams_subscribe(AmsModule module, int roleNbr, short continuumNbr,
int unitNbr, short subjectNbr, int priority, unsigned char flowLabel,
AmsSequence sequence, AmsDiligence diligence)
...
=item int ams_unsubscribe(AmsModule module, int roleNbr, short continuumNbr,
int unitNbr, short subjectNbr)

Cancels the subscription characterized by the indicated subject and
domain.  Returns 0 on success, -1 on any error.

=item int ams_publish(AmsModule module, short subjectNbr, int priority,
unsigned char flowLabel, int contentLength, char *content, int context)
...
=item int ams_send(AmsModule module, short continuumNbr, int unitNbr,
int moduleNbr, short subjectNbr, int priority, unsigned char flowLabel,
...
=item int ams_query(AmsModule module, short continuumNbr, int unitNbr,
int moduleNbr, short subjectNbr, int priority, unsigned char flowLabel,
int contentLength, char *content, int context, int term, AmsEvent *event)

Sends a message exactly as described above for ams_send(), but additionally
...
=item int ams_reply(AmsModule module, AmsEvent msg, short subjectNbr,
int priority, unsigned char flowLabel, int contentLength, char *content)

Sends a message exactly as described above for ams_send(), except that the
...
=item int ams_announce(AmsModule module, int roleNbr, short continuumNbr,
int unitNbr, short subjectNbr, int priority, unsigned char flowLabel,
int contentLength, char *content, int context)

Sends a message exactly as described above for ams_send(), except that one
...
itself characterized by a domain that includes the sending module, rather
...
=item int ams_parse_msg(AmsEvent event, short *continuumNbr, int *unitNbr,
int *moduleNbr, short *subjectNbr, int *contentLength, char **content,
...
=item int ams_parse_notice(AmsEvent event, AmsStateType *state,
AmsChangeType *change, int *unitNbr, int *moduleNbr, int *roleNbr,
short *domainContinuumNbr, int *domainUnitNbr, short *subjectNbr, int *priority,
...
AmsInvitationState, or AmsSubscriptionState depending on whether the
...
in the MIB for this module, or NULL on any error.  See lyst(3) for operations
that can be performed on the returned linked list.

=item int ams_get_continuum_nbr(void)
...
=item int ams_msgspace_is_neighbor(AmsModule module, short continuumNbr)

Returns 1 if I<continuumNbr> identifies a message space whose RAMS gateways
...
the venture within which this module is registered, or NULL on any error.
...
=item char *ams_lookup_subject_name(AmsModule module, short subjectNbr)

Returns the name of the subject identified by I<subjectNbr>, in
the venture within which this module is registered, or NULL on any error.

=item char *ams_lookup_continuum_name(AmsModule module, short continuumNbr)

Returns the name of the continuum identified by I<continuumNbr>, or NULL
on any error.
