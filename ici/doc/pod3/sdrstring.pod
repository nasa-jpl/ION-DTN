...
=head1 SYNOPSIS
    ...
    SdrObject sdr_string_create(Sdr sdr, char *from);
    SdrObject sdr_string_dup(Sdr sdr, SdrObject from);
    int sdr_string_length(Sdr sdr, SdrObject string);
    int sdr_string_read(Sdr sdr, char *into, SdrObject string);
...
=item SdrObject sdr_string_create(Sdr sdr, char *from)
...
=item SdrObject sdr_string_dup(Sdr sdr, SdrObject from)
...
=item int sdr_string_length(Sdr sdr, SdrObject string)

Returns the length of the indicated self-delimited string (as would
be returned by strlen()), or -1 on any error.

=item int sdr_string_read(Sdr sdr, char *into, SdrObject string)
