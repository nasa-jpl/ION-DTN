...
B<udplso> {I<remote_engine_hostname> | @}[:I<remote_port_nbr>] [I<txbps>] [I<token_window_time = 0>] I<remote_engine_nbr>
...
that is supported by the underlying network. Congestion control is provided
by an implementation of the token bucket algorithm. By default, a new 
token will be added every 1/txbps seconds. If your system lacks that timing
resolution, or if additional "burstiness" is desired, the token_window_time
can be set to some interval (in ms). In this case, new tokens will be
added every token_window_time milliseconds.
