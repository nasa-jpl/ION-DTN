...
APIs that preserve milliseconds use the optional suffix C<.sss>.  The
same buffer size is sufficient for both forms.

=item int ionAttach(void)
...
=item void ionProd(uvast fromFqnn, uvast fromFqnn, size_t xmitRate, unsigned int owlt)
...
=item void ionTerminate(int shutdown)

Shuts down the entire ION node, terminating all daemons, and destroys the SDR.
...
=item SdrObject ionCreateZco(ZcoMedium source, SdrObject location, vast offset,
vast length, unsigned char coarsePriority, unsigned char finePriority,
ZcoAcct acct, ReqAttendant *attendant)
...
((SdrObject) -1) is returned), or (c) either I<attendant> was null and sufficient
...
=item vast ionAppendZcoExtent(SdrObject zco, ZcoMedium source,
SdrObject location, vast offset, vast length, unsigned char coarsePriority,
...
=item int ionSendZcoByTcp(int *sock, SdrObject zco, char *buffer, int buflen)

Transmits the content of zero-copy object I<zco> to the TCP socket
identified by I<*sock>.  Transmits in chunks of size I<buflen> or less,
loading chunks of ZCO content into I<buffer> for this purpose.

=item char *getIonVersionNbr(void)

Returns the name of the ION version installed on the local machine.

=item Sdr getIonsdr(void)

Returns a pointer to the SDR management object, previously acquired by calling
ionAttach(), or zero on any error.

=item SdrObject getIonDbObject(void)

Returns the SDR heap address of the node's non-volatile ION database.

=item PsmPartition getIonwm(void)

Returns a pointer to the ION working memory partition, previously acquired
by calling ionAttach(), or zero on any error.

=item int getIonMemoryMgr(void)
...
=item IonVdb *getIonVdb(void)

Returns the address in memory of the node's volatile ION database.

=item char *getIonWorkingDirectory(void)

Returns the name of the directory in which the node resides.

=item int ionLocked(void)
...
=item uvast getOwnFqnn(void)

Returns the fully qualified node number identifying this node, as derived from
the IPN node number initialized by I<ionadmin> with or without the allocator.

=item int setDeltaFromUTC(int newDelta)

Sets the value of the UTC "delta" adjustment that ION will apply to
the system clock time order to obtain the correct UTC time.

=item int setDeltaFromUTCMs(int newDelta, int16_t newDeltaMillis)

Sets the seconds and signed millisecond components of the UTC "delta"
adjustment.  I<newDeltaMillis> must be in the range -999 through 999.
Returns zero on success and -1 on error.  setDeltaFromUTC() remains the
whole-second wrapper and sets the millisecond component to zero.

=item time_t getCtime(void)
...
=item void getCtimeMs(time_t *seconds, uint16_t *milliseconds)

Returns the current UTC-adjusted calendar time without discarding its
millisecond component.  I<*milliseconds> is normalized to the range 0
through 999.  getCtime() remains the whole-second wrapper.

=item int ionClockIsSynchronized(void)
...
=item void writeTimestampUTCMs(time_t timestamp, uint16_t milliseconds, char *timestampBuffer)

Expresses the supplied UTC seconds and millisecond component in
I<timestampBuffer>.  A non-zero component is emitted as C<.sss>; the suffix
is omitted when I<milliseconds> is zero.  I<milliseconds> must be in the
range 0 through 999.
...
=item int readTimestampUTCMs(char *timestampBuffer, time_t referenceTime, time_t *seconds, uint16_t *milliseconds)

Parses an absolute or relative UTC timestamp with an optional one-to-three
digit fractional suffix.  The fraction is scaled to milliseconds, so C<.5>,
C<.50>, and C<.500> all denote 500 milliseconds.  On success, stores the
whole seconds and a millisecond value in the range 0 through 999 and returns
zero.  Returns -1 if the timestamp is invalid.

=item int ionCompareTimestamp(time_t firstSeconds, uint16_t firstMilliseconds, time_t secondSeconds, uint16_t secondMilliseconds)

Compares two second-and-millisecond timestamps.  Returns a value less than
zero, zero, or greater than zero when the first timestamp is respectively
earlier than, equal to, or later than the second.

=item void ionShiftTimestamp(time_t baseSeconds, uint16_t baseMilliseconds, long deltaSeconds, int deltaMilliseconds, time_t *resultSeconds, uint16_t *resultMilliseconds)

Adds the signed second and millisecond delta to the base timestamp and
normalizes the result so that I<*resultMilliseconds> is in the range 0
through 999.  I<baseMilliseconds> must be in that same range.
...
bpstats(1), ionadmin(1), rfxclock(1), ams(3), bp(3), bss(3), cfdp(3), llcv(3),
ltp(3), lyst(3), memmgr(3), platform(3), psm(3), sdr(3), zco(3)
