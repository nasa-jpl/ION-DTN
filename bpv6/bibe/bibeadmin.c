...
static void	executeAdd(int tokenCount, char **tokens)
{
	...
	if (strcmp(tokens[1], "bcla") == 0)
	{
		...
		{
		case 7:
			label = strtoul(tokens[idxD], NULL, 0);
		...
		lifespan = strtoul(tokens[idxA], NULL, 0);
		priority = strtoul(tokens[idxB], NULL, 0);
		ordinal = strtoul(tokens[idxC], NULL, 0);
		bibeAdd(tokens[2], lifespan, priority, ordinal, label, flags);
		return;
	}
...
static void	executeChange(int tokenCount, char **tokens)
{
	...
	if (strcmp(tokens[1], "bcla") == 0)
	{
		...
		priority = strtoul(tokens[idxB], NULL, 0);
		ordinal = strtoul(tokens[idxC], NULL, 0);
		bibeChange(tokens[2], lifespan, priority, ordinal, label,
				flags);
		return;
	}
...
static void	printBcla(Bcla *bcla)
{
	...
	{
		isprintf(buffer, sizeof buffer, "%.255s\tlifespan: %d \
priority: %c  ordinal: %c  data label: %lu", peerEid,
			bcla->lifespan, bcla->classOfService,
			bcla->ancillaryData.ordinal,
			bcla->ancillaryData.dataLabel);
	}
	else
	{
		isprintf(buffer, sizeof buffer, "%.255s\tlifespan: %d \
priority: %c  ordinal: %c", peerEid, bcla->lifespan, bcla->classOfService,
			bcla->ancillaryData.ordinal);
	}
