...
stack.  Note that the source data object for each
ZCO extent is specified indirectly, by reference to a heap space object
reference structure or a file content lien reference structure; the
reference structures contain the actual locations of the source data
...
The total volume of file system space that may be occupied by inbound
and (separately) outbound file-sourced ZCO extents and the total volume
of SDR heap space that may be occupied by inbound and (separately) outbound
...
to ZCOs: total inbound and outbound ZCO file space occupancy and total
inbound and outbound ZCO heap occupancy are
...
occupancy total.  Doing separate accounting for inbound and outbound ZCOs
enables inbound ZCOs to be formed (for data reception purposes) even when
the total current volume of outbound ZCOs has reached its limit, and vice
versa.
...
Returns the total number of file system space bytes occupied by ZCOs (inbound or outbound) created
in this Sdr.
...
Returns 1 if the total remaining space available for ZCOs (inbound or
outbound, both heap and file) is NOT enough to contain a new extent of
the indicated length in the indicated source medium.  Returns 0 otherwise.
...
reference object if I<firstExtentSourceMedium> is ZcoFileSource and must
otherwise be the SDR location of the source data itself, and (b)
I<firstExtentOffset> indicates how many leading bytes of the source data
object should be skipped over when adding the initial source data extent
to the new ZCO.  A negative value for I<firstExtentLength> indicates that the
extent is already known not to be too large for the available ZCO space, and
the actual length of the extent is the additive inverse of this value.  On
success, returns the SDR location of the new ZCO.  Returns 0 if there is
insufficient ZCO space for creation of the new ZCO; returns ((Object) -1)
on any error.
...
actual length of the extent is the additive inverse of this value.  Returns
...
=item void zco_transmute(Sdr sdr, Object zco)

Converts an inbound ZCO to an outbound ZCO, revising all occupancy figures
accordingly.  This function is used, for example, when it is determined that
a received BP bundle must be forwarded to another node. Returns SDR location
of the ZCO object on success, 0 if there is currently too little outbound
ZCO space to contain the ZCO (in which case the ZCO has already been destroyed),
...
copied.  Returns SDR location of the new ZCO on success, 0 if there is
currently too little ZCO space to contain the new ZCO, (Object) -1 on any error.
...
Returns total data length cloned, or -1 on any error.  Note that the return
length will be less than I<length> in the event that there is currently too
little ZCO space to contain the requested cloned source data; in this case,
the clone ZCO (I<toZco>) should normally be destroyed.

=item vast zco_acct(Sdr sdr, Object zco)
