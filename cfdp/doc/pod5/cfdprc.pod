...
=item B<i> [B<entity> <entity nbr>]

The B<info> command.  This command displays CFDP configuration information.

With no arguments (B<i>), displays information about the current state of the
local CFDP entity, including the current settings of all parameters that can
be managed as described below.

When B<entity> <entity nbr> is specified (e.g., B<i entity 1>), displays
detailed information about the indicated remote CFDP entity, including its
protocol, endpoint, RTT, and checksum types.

=item B<s> 'I<UTA command>'

The B<start> command.  Starts the CFDP UT adapter task and the
CFDP clock task for the local CFDP entity.

The I<UTA command> can be:
  'bputa'        - Start BP-based UT adapter only
  'bputa proxy'  - Start BP-based UT adapter with bpcpd proxy daemon
  'tcputa'       - Start TCP-based UT adapter

When 'proxy' is specified with bputa, the bpcpd daemon is also started
to handle incoming proxy operations (get requests from remote nodes).
Note: bpcpd and bpcp cannot run simultaneously on the same node.
...
first) until the limit is no longer exceeded.  Default value is 20.
...
of seconds that a CFDP file transfer is allowed to go idle before the fault
handler for inactivity is invoked.  The default is one day.

When the default fault handler for inactivity is B<cancel> (the default since
ION 4.2.0), the transaction is canceled immediately upon inactivity timeout.
If the fault handler is set to B<ignore>, the transaction remains in the system
and is not canceled until the check cycle mechanism expires: that is, after
I<ckperiod> x I<maxtimeouts> additional seconds elapse without completion, at
which point the B<checklimit> fault handler is invoked (default: cancel).
The initial default fault handlers are compiled into the CFDP library and can
only be changed by modifying the source code in B<libcfdpP.c> and recompiling.

=item B<m throttle> I<bits_per_second>

The B<manage throttle> command.  This command establishes the maximum transmit
rate for CFDP PDUs in bits per second.  The throttle enforces a constant
transmission rate by introducing a delay between consecutive PDU transmissions.
The delay for each PDU is calculated as:

    delay = (PDU_size_in_bits / max_rate_in_bps) seconds

A value of 0 disables throttling (unlimited rate).  The default value is 0.

Note: Changes take effect immediately if the UT layer (bputa) is running.  The
setting is also persisted to the CFDP database for future restarts.

Examples:

  m throttle 1000000     # Limit to 1 Mbps (constant rate, takes effect immediately)
  m throttle 10000000    # Limit to 10 Mbps
  m throttle 0           # Disable throttle (unlimited)

=item B<x> [ B<f> ]

The B<stop> command.  This command stops the UT-layer service task for
the local CFDP engine.  By default the command is refused if any CFDP
transactions are still active (inbound or outbound), to prevent
disrupting transfers that are in progress; append B<f> (or B<-f>) to
force the stop regardless.  (Node shutdown via C<cfdpadmin .> always
stops CFDP unconditionally.)
...
The startup-test command.  Reports whether the CFDP entity is started and sets
...
Display general CFDP status and configuration parameters for the local entity.

=item i entity 1

Display detailed information about remote CFDP entity 1.
...
As of ION 4.1.3s, all CFDP parameter are loaded only once per start of
UT Layer.  Changes made to CFDP configuration via subsequent cfdpadmin
command will be stored in SDR but not loaded into running instance.

To reload CFDP configuration, one needs to stop UT Layer and restart.
As an example, to change `discard` option from 0 to 1:
...
This will stop UT Layer service, change discard setting, and restart/reload
UT Layer.
