...
void adm_ion_bp_admin_init_controls()
{
	...
	adm_add_ctrl(mid_from_value(ADM_ION_BP_ADMIN_CTRL_EGRESS_PLAN_BLOCK_MID), NULL);
	names_add_name("EGRESS_PLAN_BLOCK", "Disable transmission of bundles queued for transmission to the indicated node and reforwards all non-critical bundles currently queued for transmission to this node. This may result in some or all of these bundles being enqueued for transmission to the psuedo-node limbo.", ADM_ION_BP_ADMIN, ADM_ION_BP_ADMIN_CTRL_EGRESS_PLAN_BLOCK_MID);
	UI_ADD_PARMSPEC_1(ADM_ION_BP_ADMIN_CTRL_EGRESS_PLAN_BLOCK_MID, "plan_name", AMP_TYPE_STR);

	adm_add_ctrl(mid_from_value(ADM_ION_BP_ADMIN_CTRL_EGRESS_PLAN_UNBLOCK_MID), NULL);
	names_add_name("EGRESS_PLAN_UNBLOCK", "Re-enable transmission of bundles to the indicated node and reforwards all bundles in limbo in the hope that the unblocking of this egress plan will enable some of them to be transmitted.", ADM_ION_BP_ADMIN, ADM_ION_BP_ADMIN_CTRL_EGRESS_PLAN_UNBLOCK_MID);
