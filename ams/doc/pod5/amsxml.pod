...
numbers. This change decouples the application-layer AMS continuum number from
the transport-layer network node number. Routing information is now explicitly
defined using a mandatory Gateway Endpoint ID (gweid) for each message space,
which maps the continuum to a specific network gateway address (e.g., a Bundle
Protocol endpoint).
...
sub-elements of a single sub-element of the XML extension type B<ams_mib_load>
