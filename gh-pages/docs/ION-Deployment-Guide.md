...
## LTP Performance Assessment (ION 4.2 alpha, May 2026)

This section adds a new data point collected on the in-development
ION 4.2 alpha branch (`update-4.2-ltp-instrumentation`), using the
`demos/bench-ltp` two-node loopback driver. The measurements complement
the ION 4.1.2 results above and are intended to illustrate how the
controlling bottleneck shifts as bundle size changes.

**Test Setup:**

- Two-node loopback over UDP on a single host (`udplso`/`udplsi` on
  `127.0.0.1:2112` and `127.0.0.1:2113`).
- Contact plan rate cap: 2,000,000,000 B/s (16 Gbps) between nodes 2
  and 3 — chosen to remove the contact rate as a limiter so the
  measurement floor is software speed.
- LTP aggregation block: 100,000 B – 128,000 B (per test).
- LTP estimated export/import sessions: 4 – 8 (per test).
- SDR in DRAM only (config flag 1); 20,000,000 heap words per node;
  50 MB working memory per node.
- ION rebuilt with `--enable-crypto-mbedtls --enable-bpsec-debugging`
  (default release flags).
- POSIX named semaphores (the default for modern Linux ION builds).
- Bundles generated by `bpdriver`, counted by `bpcounter`; each test
  measures steady-state throughput from the 90th-percentile fastest
  bundle window (the driver discards the first/last 5% as warmup/drain).
- ION is fully torn down (`killm`) and restarted between every test
  point to eliminate cross-test SDR state effects.

**Kernel UDP Buffers:**

UDP receive/send buffers were raised above the Linux factory default
(`net.core.rmem_max = 212992 ≈ 200 KB`) to 16 MB / 8 MB, matching the
required minimum enforced by `demos/bench-ltp/dotest` and the values
applied by `ltp_sysctl_tune.sh on`:
...
A separate spot-check run at the factory defaults was abandoned after
three of nine test points: at 2 Gbps target rate the ~200 KB socket
buffer dropped enough UDP segments that LTP spent most of each test
window retransmitting, and most test points hit the per-test 120 s
ceiling without reaching steady state. Benchmarking ION's processing
ceiling at factory-default UDP buffers is therefore not practical;
the kernel buffer must be raised first (see the warning emitted by
`dotest` if you try).

**Hardware Specification and Operating System:**

- Intel Xeon w7-3465X (Sapphire Rapids, 2023 era), 28 cores / 56 threads.
- 62 GiB RAM.
- Linux 6.6 (WSL2 kernel build, `x86_64`).
- Loopback only — no physical Ethernet. Throughput numbers therefore
  represent ION's BP/LTP/UDP software pipeline running against the
  kernel's loopback path, not a NIC-bound measurement.

### Throughput vs. Bundle Size at Two Segment Sizes

For each bundle size we ran the same workload at LTP segment size
64,000 B (a typical "space-link" segment well above any terrestrial
MTU) and at 1,400 B (just under a 1,500-byte Ethernet MTU, the
practical ceiling on terrestrial deployments). All other parameters
were held constant.

| Bundle Size (B) |  64K Segments (Mbps) | 1,400 B Segments (Mbps) | 64K Advantage |
|----------------:|---------------------:|------------------------:|--------------:|
...
### Two Regimes: Bundle-Bound at the Bottom, Segment-Bound at the Top

The two columns above tell a single story when read together. At
small bundles the per-bundle bookkeeping (BP source/sink, ZCO
plumbing, SDR transactions, LTP session setup/teardown) dominates
total time, so the choice of LTP segment size barely matters — at
1,000 B bundles the gap between 64K and 1,400 B segments is only
1.05 ×, and at 5,000 B bundles it is 1.19 ×. This is the
**bundle-bound regime**: throughput is rate-limited by how fast ION
can shepherd bundles through the BP/LTP path, regardless of how
each bundle is fragmented into segments.

As bundle size grows, per-bundle overhead is amortized across
progressively more payload, and the cost of LTP segment framing,
UDP `sendto()` calls, and per-segment ACK accounting takes over.
At 5,000,000 B bundles the gap reaches 2.90 ×; the absolute throughput
with 64K segments (2.37 Gbps) approaches the 16 Gbps contact-plan cap,
while the 1,400 B run plateaus near 820 Mbps because each bundle now
costs roughly 45 × more `sendto()` calls than the 64K-segment case
for the same payload. This is the **segment-bound regime**: per-segment
processing overhead, not bundle handling, sets the ceiling.

For the 64K column, the curve flattens around 2.1 – 2.4 Gbps once
bundles exceed 500 KB — the remaining headroom would have to come from
a larger contact-plan rate, more parallelism, or a faster CPU pipeline,
not from further LTP tuning. For the 1,400 B column, the plateau is
near 800 Mbps for any bundle ≥ 1 MB, indicating that on this hardware
1,400-byte LTP segments are syscall-bound regardless of how large the
bundles get.

### Deployment Takeaway

Two practical implications follow from this dataset:

1. **Pick the segment size that matches the link, then size bundles
   accordingly.** Terrestrial deployments often have to live with
   MTU-sized segments (≈ 1,400 B); pushing bundle size higher beyond
   ~1 MB yields no further gain on this hardware. Space-link
   deployments that can use large CCSDS encapsulation packets (64 KB
   and up) gain dramatically from large bundles, with the bulk of the
   benefit captured by the time bundles reach ~500 KB.

2. **Tuning LTP segment size in isolation is rarely the right lever
   for low-throughput regimes.** If small bundles dominate the
   workload, the system is bundle-bound and the segment-size knob will
   move the result by less than 20 %. Improvements there come from
   bundle aggregation, ZCO reuse, and reducing per-bundle SDR
   transactions — i.e. work above the LTP layer.

The numerical values above are specific to a 2023-era Xeon running
on the loopback path; they are intended to illustrate the regime
crossover, not to set a generalizable ceiling. Users running on
flight-grade processors or over real network paths should expect
absolute throughput to be lower but should expect the **same
qualitative shape**: a bundle-bound floor at small bundle sizes
where segment size barely matters, and a segment-bound ceiling at
large bundle sizes where segment size dominates.
