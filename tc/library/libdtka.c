/*
	libtca.c:	common functions for Trusted Collective
...
int	tca_serialize(unsigned char *buffer, unsigned int buflen,
...
int	tca_deserialize(unsigned char **cursor, int *bytesRemaining,
		...
		unsigned short *datLength, unsigned char *datValue) 
{
	...
	{
		writeMemo("Malformed TCA record: too short.");
		return 0;
	}
	...
	*datLength = ntohs(*datLength);
	...
	{
		writeMemoNote("Malformed TCA record: key is too long",
				utoa(*datLength));
		return 0;
	}
	...
	{
		writeMemo("Malformed TCA record: key truncated.");
		return 0;
	}
