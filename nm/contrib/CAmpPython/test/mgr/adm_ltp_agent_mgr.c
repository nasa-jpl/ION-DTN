...
void adm_ltp_agent_init_reports()
{
	...
	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_REMOTE_ENGINE_NBR_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_CUR_EXPT_SESS_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_CUR_OUT_SEG_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_CUR_IMP_SESS_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_CUR_IN_SEG_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_RESET_TIME_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_SEG_Q_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_SEG_Q_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_SEG_POP_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_SEG_POP_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_CKPT_XMIT_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_POS_ACK_RX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_NEG_ACK_RX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_CANCEL_RX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_CKPT_REXMIT_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_CANCEL_XMIT_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_OUT_COMPLETE_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_RED_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_RED_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_GREEN_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_GREEN_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_REDUNDANT_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_REDUNDANT_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_MAL_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_MAL_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_UNK_SENDER_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_UNK_SENDER_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_UNK_CLIENT_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_RX_UNK_CLIENT_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_STRAY_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_STRAY_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_MISCOLOR_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_MISCOLOR_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_CLOSED_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_SEG_CLOSED_BYTES_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_CKPT_RX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_POS_ACK_TX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_NEG_ACK_TX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_CANCEL_TX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_ACK_RETX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_CANCEL_RX_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);

	cur_mid = mid_from_value(ADM_LTP_AGENT_EDD_SPAN_IN_COMPLETE_CNT_MID);
	mid_add_param_from_value(cur_mid, val_from_uint(0));
	cur_item = rpttpl_item_create(cur_mid, 1);
	rpttpl_item_add_parm_map(cur_item, 1, 1);
	lyst_insert_last(rpt,cur_item);
