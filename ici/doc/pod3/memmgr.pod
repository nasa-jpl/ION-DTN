...
=item int memory_open(int memKey,
                      unsigned long memSize,
                      char **memPtr,
                      int *smId,
                      char *partitionName,
...
memory_open() will allocate system memory as necessary and will in
...
PSM management, then memory_open() will invoke psm_manage() to manage
the shared memory region.  It will also add a catalogue for the managed
shared memory region as necessary.

If I<memMgr> is non-NULL, then memory_open() will additionally call
...
=item void memory_destroy(int smId, PsmPartition *partition);

memory_destroy() terminates all access to a PSM managed region of shared
