...
=item B<a listen> I<listen_socket_spec>

Listen socket specification command. This command asserts, in the form
I<IP_address>:I<port_number>, the specification for a socket at which the
IPND daemon is to listen for incoming beacons. The address can be an unicast,
