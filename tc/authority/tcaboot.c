		...
		int a6, int a7, int a8, int a9, int a10)
{
	...
	int	fec_K = a3 ? atoi((char *) a3) : 50;
	doubtle	fec_R = a4 ? atof((char *) a4) : .2;
	int	delay = a5 ? atoi((char *) a5) : 5;
#else
int	main(int argc, char **argv)
{
	int	bulletinsGroupNbr = argc > 1 ? atoi(argv[1]) : -1;
	int	recordsGroupNbr = argc > 2 ? atoi(argv[2]) : -1;
	int	fec_K = argc > 3 ? atoi(argv[3]) : 50;
	double	fec_R = argc > 4 ? atof(argv[4]) : .2;
	int	delay = argc > 5 ? atoi(argv[5]) : 5;
#endif
	int	fd;
	time_t	currentTime;
