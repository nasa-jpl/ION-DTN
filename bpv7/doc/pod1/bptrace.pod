...
B<bptrace> transparently handles both traditional BPv7 status reports
(admin record type 1) and Compressed Reporting Signals (CRS, admin record
type 14) as defined in the CCSDS Orange Book specification.  When nodes
in the network are configured with B<m custodymode orangebook>, they will
generate CRS instead of traditional status reports.  B<bptrace> automatically
detects the report type and displays the status information in a unified
format.

B<Listen-only mode:> When invoked with the B<-listen> flag, B<bptrace> opens
the specified endpoint and displays bundle status reports as they arrive, without
sending any trace bundles.  This mode is useful for monitoring status reports from
bundles sent by other applications.  The program will continue listening until
interrupted (Ctrl+C) or until 128 reports have been received.  Status reports are
...
(bulk) or 1 (standard) or 2 (expedited), I<ordinal> must be 0-254 (ION supports
full 8-bit range; 255 is reserved), I<unreliable> must be 0 or 1 (Boolean),
I<critical> must also be 0 or 1 (Boolean), and I<data-label> may be any
unsigned integer.  I<custody-requested>
...
NOTE: The Quality of Service (QoS) extension block (type 254) is an
ION-specific, non-standard extension. It is patterned after the IETF
draft-burleigh-dtn-ecos-00 Extended Class of Service (ECOS) specification,
but uses a different wire format. ION's QoS block encodes classOfService
and ordinal as separate CBOR integers in a 4-element array, whereas the
IETF draft specifies a 5-element array with priority and ordinal bit-packed
into a single byte. This block will not interoperate with implementations
...
destination endpoint, but when used in conjunction with priority 2.63 it
...
B<-listen> runs bptrace in listen-only mode.  In this mode, only I<listenEid> is
required.  B<bptrace> will open the endpoint and display status reports as they
arrive in real-time, without sending any bundles.  The B<-v> flag can be used
for verbose debug output.  The program listens indefinitely until interrupted
(Ctrl+C) or until 128 reports have been received.
...
=head1 CRS (COMPRESSED REPORTING SIGNALS) SUPPORT

B<bptrace> supports both traditional BPv7 status reports and Compressed
Reporting Signals (CRS) as specified in the CCSDS Orange Book "Compressed
Bundle Status Reporting and Custody Signaling" specification.

When a node is configured with B<m custodymode orangebook> in its bprc file,
bundles with status report requests will receive a CREB (Compressed Reporting
Extension Block) instead of setting the traditional BPv7 status report request
flags.  Intermediate nodes generate CRS admin records (type 14) instead of
traditional status reports (type 1).

B<bptrace> automatically detects the admin record type and processes both
formats identically.  The displayed output uses a unified format regardless
...
Note that CRS does not carry timestamp information, so status times shown
for CRS reports reflect the local receive time rather than the time the
status event occurred at the remote node.
