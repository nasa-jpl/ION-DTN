...
int	main(int argc, char *argv[])
{
   ...
   {
      PUTS("Usage: aoslso {<remote engine's host name> | @}[:<its port number>] <txbps (0=unlimited)> <remote engine ID>");
      return 0;
   }
   ...
   /*	Can now begin transmitting to remote engine.		*/
   {
      ...
      isprintf(txt, sizeof(txt), "[i] aoslso is running, spec=[%s:%d], txbps=%d (0=unlimited), rengine=%lu.", 
	      inet_ntoa(inetName->sin_addr), ntohs(portNbr),
	      txbps, remoteEngineId );
   ...
   while (running && !(sm_SemEnded(vspan->segSemaphore)))
   {
	 ...
	 if (bytesSent < segmentLength)
	 {
      ...
      {
	 unsigned int usecs;
	 float sleep_secs = (1.0 / ((float)txbps)) * ((float)(bytesSent*8));
