...
Same as `cfdp_rls` except that `listingOptions` may be used to request extended directory listing behavior. `CFDP_DIRLIST_OPTION_STATUS_BYTES` causes the responder to produce a v2 manifest in which each entry is prefixed by a one-byte status flag and a one-byte type indicator (`CFDP_DIRENT_REGULAR`, `CFDP_DIRENT_DIRECTORY`, `CFDP_DIRENT_SYMLINK`, `CFDP_DIRENT_OTHER`, or `CFDP_DIRENT_UNKNOWN`), and the response carries an "incomplete" flag (in `CfdpDirListingResponse.directoryListingIncomplete`) indicating whether the responder dropped any entries. `CFDP_DIRLIST_OPTION_RECURSIVE` requests a manifest for the subtree rooted at `dirListTask->directoryName`; directory entries in recursive manifests are identified by a trailing `/`. The status-byte and recursive options may be combined.

Extended requests are wire-compatible with old responders: option bits are encoded as an optional trailing byte after `destFileName` in the existing type-16 request body, which old parsers stop reading before. Old responders therefore reply with a legacy (type 17) response and the new requestor sees a legacy manifest.
