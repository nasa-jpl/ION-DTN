...
access.  Note also that "transmitting" or "receiving" a ZCO updates
progress counters in the ZCO reference object and therefore is an
SDR data modification that must occur in the course of an SDR
transaction.
...
without copying.  Returns number of bytes copied (or skipped) on success,
0 on any file access error, -1 on any other error.
