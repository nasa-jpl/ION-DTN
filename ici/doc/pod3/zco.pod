...
=item void zco_append_extent(Sdr sdr, Object zcoRef, ZcoMedium sourceMedium, Object location, unsigned int offset, unsigned int length)
...
=item void zco_prepend_header(Sdr sdr, Object zcoRef, char *header, unsigned int length)

=item void zco_append_trailer(Sdr sdr, Object zcoRef, char *trailer, unsigned int length)
...
Attaching a header or trailer causes it to be written to the SDR.
...
themselves; it includes the ZCO's share of any applicable ZCO file
reference overhead (the number of bytes of SDR heap space occupied by
any ZCO file reference objects that the extents of the ZCO point to);
and it B<omits> the sizes of all source data extents that do not occupy
SDR heap space (i.e., ones that reside in files to which ZCO extent
objects refer).
...
(referenced by I<reader>) into I<buffer>.  Returns the number of bytes
...
I<buffer>.  Returns number of bytes copied, or -1 on any error.
