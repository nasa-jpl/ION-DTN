...
=item int psm_manage(char *start, size_t length, char *name, PsmPartition *partitionPointer, PsmMgtOutcome *outcome)
...
=item PsmAddress psm_malloc(PsmPartition partition, size_t length)
...
=item PsmAddress psm_zalloc(PsmPartition partition, size_t length)
...
=item int psm_start_trace(PsmPartition partition, size_t traceLogSize, char *traceLogAddress)
