...
B<cfdptest> [I<destination entity nbr> I<source file name> I<destination file name> [I<time-to-live> [I<priority> [I<ordinal> [I<unreliable> [I<critical>]]]]]]
...
PDUs are sent with custody transfer requested and all bundle status reporting
disabled.

When command-line parameters are used to specify destination entity number,
source file name, destination file name, and (optionally) quality-of-service
paramter values, B<cfdptest> uses those values to initiate transmission of
the indicated file and then immediately terminates.  Omitted parameters
default as noted below.

Otherwise, B<cfdptest> offers the user an interactive "shell" for testing CFDP
file transmission.  B<cfdptest> prints a prompt string (": ") to stdout,
accepts strings of text from stdin, and uses the strings to configure and
initiate a CFDP file transmission operation.  Each string of text from stdin
is interpreted as a command comprising several tokens: a one-character
command code and, in most cases, one or more command argument of one or
more characters.

The supported interactive B<cfdptest> commands are as follows:
...
CFDP PDUs.  Valid values are 0 (assured, reliable) and 1 (best-effort,
unreliable).  If not specified, transmission mode is 0.
...
by the most recent preceding B<d>, B<f>, and B<t> commands.
