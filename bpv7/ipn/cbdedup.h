/*
	...
	A critical bundle is forwarded over every possible route
	to the destination endpoint.

	(Not flooded across all CGR-suitable neighbors at every hop.)

	Without de-duplication, a single critical bundle can replicate
	exponentially in dense topologies where multiple routes converge
	on and emerge from a single forwarding node.

	This module keeps a seen-set in ION working memory keyed by
	bundle ID; once a critical bundle has been forwarded out of
	this node, any subsequent copy received here is dropped
	instead of re-exploded.
