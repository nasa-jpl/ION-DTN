...
istrcpy() is a safe implementation of strncpy(); see the strncpy(P) man
page for details.  istrcpy() differs from strncpy() in that it always
NULL-terminates the string in I<buffer>, even if the length of I<sourceString>
string would equal or exceed I<bufSize> (in which case I<sourceString> is
truncated).
...
of igetswd() must be supplied in an included file named "wdname.c"; this
...
=item int igets(int fd, char *buffer, int buflen)
...
the line are written into I<buffer>.  On error the function returns -1;
otherwise the function returns the length of the text line in I<buffer>,
as if from strlen(3).
...
adhere to) is as follows:
	...
	When returning zero (indicating successful completion of the
	function) -- even if the result might be interpreted as an
	operational failure at a higher level (e.g., an object
	identified by a given string is not found, through no
	failure of the search function) -- do NOT invoke putErrmsg().

	Use putErrmsg() and putSysErrmsg() only when functions are
	unable to proceed to normal completion.  Use writeMemo()
	or writeMemoNote() if you want to log a message.

	Whenever returning -1 (indicating function failure):
		...
		layer of the call stack; simply use putSysErrmsg (or
		...
		that was detected locally by code within the function
		that has failed, or the failure is due to the reported
		failure of some other ION function -- then use
		putErrmsg (or postErrmg if pressed for time) to
		describe the nature of the failure condition.  This
		will aid in tracing the failure through the function
		stack in which the failure was detected.  The text of
		the error message should normally start with a capital
		letter and should end with a period.
...
Returns a brief text message describing the current system error, as identified
