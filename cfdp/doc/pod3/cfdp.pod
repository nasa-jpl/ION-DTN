...
service class defined in the CFDP Blue Book.
...
=item void cfdp_compress_number(CfdpNumber *toNbr, unsigned long from)

Converts a number in native unsigned long integer form into a CfdpNumber
structure, e.g., for use when invoking the cfdp_put() function.

=item void cfdp_decompress_number(unsigned long toNbr, CfdpNumber *from)

Converts a numeric value in a CfdpNumber structure to an unsigned long integer.

=item void cfdp_update_checksum(unsigned char octet, unsigned int *offset, unsigned int *checksum)
...
request number on success, -1 on any error.
...
=item int cfdp_preview(CfdpTransactionId *transactionId, unsigned int offset, int length, char *buffer);
...
so far is returned as the new value of I<extentCount>.  On user error
(transaction is nonexistent or is outbound), returns 0.  On system failure,
