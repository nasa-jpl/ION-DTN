...
adhere to) is as follows:
	...
	must be returned as the value of a reference argument.
	[Note that the SDR hash table management functions deviate
	from this rule, returning 0 to indicate nominal completion
	but functional failure (duplicate key or key/value pair not
	found) and returning 1 to indicate functional success.]
