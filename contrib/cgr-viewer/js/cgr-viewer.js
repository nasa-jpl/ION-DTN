...
function fillRoute(box, route, constants) {
  ...
  $(".dispatchTime", box).text(fromMoment.format(TIME_FMT));
  $(".deliveryTime", box).text(deliveryMoment.format(TIME_FMT));
  $(".maxCapacity", box)
    .text(simplifySize(route.maxCapacity) +
  ...
  if (route.flag == constants.SELECTED)
    reason.text("Best route found");
  else
    reason.text(cap(route.ignoreReason));
  ...
  case constants.CONSIDERED:
    heading.text("Considered");
  break;
  case constants.IDENTIFIED:
    heading.text("Not Considered");
  break;
  default:
...
function handleParams(params) {
  ...
  paramRoute.text("Node " + params.localNode + " to Node " +
    params.destNode);
  paramDispatchTime.text(dispatchMoment.format(TIME_FMT));
  paramExpTime.text(expMoment.format(TIME_FMT));
  paramBundleSize
    .text(simplifySize(params.bundleSize))
    .attr("title", params.bundleSize + " bytes");
  paramBundleClass.text(params.minLatency ? "Minimum Latency"
                                          : "Best Effort");
}

// Validate JSON structure before processing
function validateJSON(cgr) {
  // Check top-level structure
  if (!cgr || typeof cgr !== 'object') {
    return "Invalid JSON: not an object";
  }

  if (!cgr.constants || typeof cgr.constants !== 'object') {
    return "Invalid JSON: missing or invalid 'constants'";
  }

  if (!cgr.params || typeof cgr.params !== 'object') {
    return "Invalid JSON: missing or invalid 'params'";
  }

  if (!Array.isArray(cgr.routes)) {
    return "Invalid JSON: missing or invalid 'routes' array";
  }

  // Validate params structure
  var requiredParams = ['localNode', 'destNode', 'dispatchTime', 'expirationTime', 'bundleSize'];
  for (var i = 0; i < requiredParams.length; i++) {
    if (!(requiredParams[i] in cgr.params)) {
      return "Invalid JSON: missing param '" + requiredParams[i] + "'";
    }
  }

  // Validate each route has required fields
  for (var i = 0; i < cgr.routes.length; i++) {
    var route = cgr.routes[i];
    if (typeof route.flag === 'undefined' ||
        typeof route.fromTime === 'undefined' ||
        typeof route.deliveryTime === 'undefined' ||
        typeof route.graph === 'undefined') {
      return "Invalid JSON: route " + i + " missing required fields";
    }
  }
...
function handleJSON(cgr) {
  var error = validateJSON(cgr);
  if (error) {
    unblurBackground();
    alert("Error loading CGR data: " + error + "\n\nPlease ensure you're loading a valid JSON file generated by cgrfetch.");
    return false;
  }
...
fileChooser.change(function(e) {
  ...
  reader.addEventListener("load", function(e) {
    ...
    var success = false;
    try {
      var cgr = JSON.parse(reader.result);
      success = handleJSON(cgr);
    } catch (err) {
      unblurBackground();
      alert("Error parsing JSON file: " + err.message + "\n\nPlease ensure you're loading a valid JSON file.");
      return;
    }
