...
B<owltsim> delays delivery of data between pairs of BP nodes by specified
...
=item I<to> identifies the receiving node, an ipn-scheme node number.

This parameter is purely informational, intended to make B<owltsim>'s
printed messages more helpful to the user.  The ipn-scheme allocator
number for this node is always implicitly zero.
...
more helpful to the user, and again the ipn-scheme allocator
number for this parameter is always implicitly zero.
...
This file indicates that B<owltsim> will receive on port 5502 the BP
...
In the reverse direction, B<owltsim> will receive on port 5507 the BP
