...
In multi-node mode, `killm f` deliberately **skips** graceful `ionexit` and goes straight to SIGTERM → SIGKILL across all ION processes, followed by unconditional `ipcrm`/semaphore cleanup. It does **not** walk the node directories or run `ionexit` per node. If you need an orderly shutdown that keeps other nodes running, use per-node `ionexit k n` (see the [ION Shutdown Guide](shutdown-guide.md)) instead of `killm f`.
...
Those rules, the per-path breakdown of what `killm` does with and without `f`, and the reasoning behind them live in the [ION Shutdown Guide](shutdown-guide.md); they are deliberately not duplicated here.
