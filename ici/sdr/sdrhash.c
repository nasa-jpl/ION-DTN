		...
		int estNbrOfEntries, int meanSearchLength)
{
	...
	if (table == 0)
	{
		_putErrmsg(file, line, "can't create hash table",
				itoa(rowCount));
		return 0;
	}
	...
	for (i = 0; i < rowCount; i++)
	{
		...
		if (listAddr == 0)
		{
			_putErrmsg(file, line, "Can't populate hash table row.",
					NULL);
			crashXn(sdrv);
			return 0;
		}
		...
		Address value)
{
	...
	{
		errno = EINVAL;
		_putErrmsg(file, line, badAddressMsg, "hash");
		crashXn(sdrv);
		return -1;
	}
