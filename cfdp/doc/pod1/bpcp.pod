...
important implications. First, file permissions are not transferred. Files will
be made executable on copy. Second, symbolic links are ignored for local to
remote transfers and their target is copied for remote transfers. Third, all
hosts must be specified using ION's IPN naming scheme.
