...
void adm_agent_init_atomic()
{
	...
	adm_add_datadef(ADM_AGENT_AD_CURTIME_MID, DTNMP_TYPE_TS,   0, agent_get_curtime, NULL, NULL);
	...
	adm_add_datadef(ADM_AGENT_AD_CURTIME_MID, DTNMP_TYPE_TS,   0, NULL, NULL, NULL);
...
void adm_agent_init_computed()
{
	...
	expr_t *expr = expr_create(DTNMP_TYPE_UINT, def);

	adm_add_computeddef(ADM_AGENT_CD_NUMRULE_MID, DTNMP_TYPE_EXPR, expr);
	expr_release(expr);

//	midcol_destroy(&def);

#ifndef AGENT_ROLE
	adm_add_datadef(ADM_AGENT_CD_NUMRULE_MID, DTNMP_TYPE_UINT, 1, NULL, adm_print_unsigned_long, adm_size_unsigned_long);
...
void adm_agent_init_controls()
{
    ...
    adm_add_ctrl(ADM_AGENT_CTL_STOR_MID,    agent_ctl_stor);

    adm_add_ctrl(ADM_AGENT_CTL_RESET_CNTS,  agent_ctrl_reset_cnt);
...
//	ui_add_parmspec(ADM_AGENT_CTL_LSTMAC_MID, 0, 0, 0, 0, 0, 0);
	...
	ui_add_parmspec(ADM_AGENT_CTL_ADDTRL_MID, 5, "TRL ID", DTNMP_TYPE_MID, "Timestamp", DTNMP_TYPE_SDNV, "Period", DTNMP_TYPE_SDNV, "Count", DTNMP_TYPE_SDNV, "Action", DTNMP_TYPE_MC);
...
//	ui_add_parmspec(ADM_AGENT_CTL_LSTTRL_MID, 0, 0, 0, 0, 0, 0);
	...
	ui_add_parmspec(ADM_AGENT_CTL_ADDSRL_MID, 5, "SRL ID", DTNMP_TYPE_MID, "Timestamp", DTNMP_TYPE_SDNV, "State Expr", DTNMP_TYPE_EXPR, "Count", DTNMP_TYPE_SDNV, "Action", DTNMP_TYPE_MC);
...
//	ui_add_parmspec(ADM_AGENT_CTL_LSTSRL_MID, 0, 0, 0, 0, 0, 0);
	...
	ui_add_parmspec(ADM_AGENT_CTL_STOR_MID, 2, "CD MID", DTNMP_TYPE_MID, "Expression", DTNMP_TYPE_EXPR, NULL, 0, NULL, 0, NULL, 0);
...
//    result.length = sizeof(uint32_t);
 ...
 //   result.length = 0;
	...
	ui_add_parmspec(ADM_AGENT_LIT_USRVAST_MID, 1, "8 byte VAST", DTNMP_TYPE_VAST, NULL, 0, NULL, 0, NULL, 0, NULL, 0);

	names_add_name("ADM_AGENT_LIT_USRUVAST_MID", "User-Defined Unsigned Vast", ADM_AGENT, ADM_AGENT_LIT_USRUVAST_MID);
	ui_add_parmspec(ADM_AGENT_LIT_USRUVAST_MID, 1, "UVAST", DTNMP_TYPE_SDNV, NULL, 0, NULL, 0, NULL, 0, NULL, 0);

	names_add_name("ADM_AGENT_LIT_USRFLT_MID", "User-Defined Float", ADM_AGENT, ADM_AGENT_LIT_USRFLT_MID);
	ui_add_parmspec(ADM_AGENT_LIT_USRFLT_MID, 1, "Serialized Float", DTNMP_TYPE_BLOB, NULL, 0, NULL, 0, NULL, 0, NULL, 0);

	names_add_name("ADM_AGENT_LIT_USRDBL_MID", "User-Defined Double", ADM_AGENT, ADM_AGENT_LIT_USRDBL_MID);
	ui_add_parmspec(ADM_AGENT_LIT_USRDBL_MID, 1, "Serialized Double", DTNMP_TYPE_BLOB, NULL, 0, NULL, 0, NULL, 0, NULL, 0);
...
value_t adm_agent_user_vast(mid_t *id)
{
	...
	if(success <= 0)
	{
		DTNMP_DEBUG_ERR("adm_agent_user_int","Can't grab first parameter.",NULL);
		return result;
	}
...
value_t adm_agent_user_uvast(mid_t *id)
{
	...
	if(success <= 0)
	{
		DTNMP_DEBUG_ERR("adm_agent_user_uvast","Can't grab first parameter.",NULL);
		return result;
	}
...
value_t adm_agent_user_float(mid_t *id)
{
	...
	if((entry = adm_extract_blob(id->oid.params, 0, &success)) == NULL)
	{
		...
		{
			DTNMP_DEBUG_ERR("adm_agent_user_float","User length %d > Data len %d.",entry->length,  sizeof(float));
		}
	}
	else
	{
...
value_t adm_agent_user_double(mid_t *id)
{
	...
	if((entry = adm_extract_blob(id->oid.params, 0, &success)) == NULL)
	{
		...
		{
			DTNMP_DEBUG_ERR("adm_agent_user_double","User length %d > Data len %d.",entry->length, sizeof(double));
		}
	}
	else
	{
