 ...
 *	characterized by a node that accepts the messages on a given
 *	subject and the subject on which messages are accepted by that
 *	node.  The rule constrains senders of this messages, by role
 *	and/or unit, and it states the selected endpoint for sending
 *	messages on this subject to this node (based on the rule's
 *	implicit service mode, which was specified by the node), the
 ...
 *	relationship (i.e., within a given InterestedNode and the
 ...
 *	messages.  The XmitRules recorded by a node may well include
 *	rules that apply only to nodes in other message spaces.  But
...
typedef struct
{
	...
	InterestedNode	*node;		/*	Back-reference.		*/
	int		roleNbr;	/*	Msg source constraint.	*/
	int		continuumNbr;	/*	Msg source constraint.	*/
	int		unitNbr;	/*	Msg source constraint.	*/
	int		priority;	/*	Requested by node.	*/
	unsigned char	flowLabel;	/*	Requested by node.	*/
...
/*	AmsInterface is a structure used by the LOCAL node (SAP) to
 ...
 *	AMS messages to a REMOTE node on a specified service mode,
 *	as selected from among the endpoints in that node's delivery
...
/*	A DeliveryVector lists the interfaces at which the LOCAL node
 ...
 *	priority, and flow label other nodes are to use when sending
 *	messages on the indicated subject to the LOCAL node (the SAP)
 *	- if in fact such transmission is supported at all.  MsgRules
 *	are the source of the information encapsulated in other nodes'
 *	XmitRules.
 *
...
typedef struct
{
	...
	int		unitNbr;	/*	Msg source constraint.	*/
	int		priority;	/*	Requested by node.	*/
	int		flowLabel;	/*	Requested by node.	*/
