...
SDR persistent data are referenced in application code by SdrObject values and
SdrAddress values, both of which are simply displacements (offsets) within SDR
address space.  The difference between the two is that an SdrObject is always
the address of a block of heap space returned by some call to sdr_malloc(),
while an SdrAddress can refer to any byte in the address space.  That is, the
value of an SdrObject is a valid SdrAddress, but an SdrAddress does not
...
=item int sdr_initialize(size_t wmSize, char *wmPtr, int wmKey, char *wmName)
...
=item int sdr_load_profile(char *name, int configFlags, size_t heapWords,
int heapKey, size_t logSize, int logKey, char *pathName, char *restartCmd)
...
to execute this task must be provided in I<restartCmd>. If I<restartCmd> is NULL
then no cleanup task will be run upon transaction reversal.

Returns 0 on success, -1 on any error.

=item int sdr_reload_profile(char *name, int configFlags, size_t heapWords,
int heapKey, size_t logSize, int logKey, char *pathName, char *restartCmd)
...
=item size_t sdr_heap_size(Sdr sdr)

Returns the total size of the SDR heap, in bytes.

=item void sdr_stop_using(Sdr sdr, int shutdown)
...
=item void sdr_destroy(Sdr sdr, int shutdown)
...
=item int sdr_begin_xn(Sdr sdr)

Initiates a transaction.  Returns 1 on success, 0 on any failure.  Note
that transactions are single-threaded; any task that calls sdr_begin_xn()
...
=item void sdr_read(Sdr sdr, char *into, SdrAddress from, size_t length)
...
=item void sdr_write(Sdr sdr, SdrAddress into, char *from, size_t length)
...
=item void *sdr_pointer(Sdr sdr, SdrAddress address)
...
=item SdrAddress sdr_address(Sdr sdr, void *pointer)
...
=item SdrObject sdr_malloc(Sdr sdr, size_t size)
...
=item SdrObject sdr_insert(Sdr sdr, char *from, size_t size)
...
=item SdrObject sdr_stow(sdr, variable)
...
=item size_t sdr_object_length(Sdr sdr, SdrObject object)

Returns the number of bytes of heap space allocated to the application
data at I<object>.

=item void sdr_free(Sdr sdr, SdrObject object)

Frees for subsequent re-allocation the heap space occupied by I<object>.

=item void sdr_stage(Sdr sdr, char *into, SdrObject from, size_t length)
...
object's content is copied into memory; I<into> may be NULL
in this case.

B<When SDR_BOUNDED is set>, every sdr_write() is validated against the
set of objects registered by sdr_stage() (and sdr_malloc()) in the
current transaction.  Any sdr_write() to an object that was only read
with sdr_read() -- not staged -- will be rejected as a boundary
violation, aborting the transaction.  Therefore:

=over 4

=item *

Use B<sdr_stage()> instead of sdr_read() whenever the object will be
modified and written back with sdr_write() in the same transaction.

=item *

Use B<sdr_stage(sdr, NULL, object, 0)> to register a pre-allocated
buffer (e.g., one obtained from sdr_malloc() in a prior transaction)
for writing in the current transaction, without copying its contents
into local memory.  This is appropriate for large I/O buffers where
only a portion of the object will be written via an offset
(e.g., C<sdr_write(sdr, bufferObj + offset, data, len)>).

=back

=item size_t sdr_unused(Sdr sdr)
...
The C<largestFreeBlock> field is the size, in user-data bytes, of the
largest contiguous free block found in either pool.  It does not
include the unassigned gap between the pools, which is reported
separately as C<unusedSize>.  Together they answer "what is the
largest single allocation this SDR can currently satisfy?":
C<max(largestFreeBlock, unusedSize - large_block_overhead)>.
...
=item int sdr_start_trace(Sdr sdr, size_t traceLogSize, char *traceLogAddress)
...
=item void sdr_catlg(Sdr sdr, char *name, int type, SdrObject object)
...
=item SdrObject sdr_find(Sdr sdr, char *name, int *type)

Locates the SdrObject associated with I<name> in the indicated SDR's catalogue
...
=item SdrObject sdr_read_catlg(Sdr sdr, char *name, int *type,
