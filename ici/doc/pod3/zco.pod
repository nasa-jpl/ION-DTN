...
Returns 1 if the total remaining file system space available for ZCOs is
greater than I<length>.  Returns 0 otherwise.

=item int zco_enough_heap_space(Sdr sdr, vast length)

Returns 1 if the total remaining SDR heap space available for ZCOs is
greater than I<length>.  Returns 0 otherwise.
...
=item vast zco_clone_extents(Sdr sdr, Object toZco, Object fromZco, vast offset, vast length)

Appends to I<toZco> a copy of a subset of the source data of I<fromZCO>.
Portions of the source data extents of I<toZCO> are copied as necessary.
...
known header or trailer data.  Use this function before starting the
transmission of a ZCO that was received from an underlying protocol layer
rather than from an overlying application or protocol layer, and before
enqueuing the ZCO for reception by an overlying application or protocol layer.
