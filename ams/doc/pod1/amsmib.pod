...
B<amsd> processes may run AMS modules in background threads, and because a
...
B<amsmib> then reads into a memory buffer up to 4096 bytes of MIB update
...
intended recipient modules were configured with the --with-expat option.
