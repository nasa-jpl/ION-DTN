...
the bundles back (for instance, it is a L<bpecho> endpoint), B<bping> will
print the round-trip time.  When complete, bping will print statistics before
exiting.  It is very similar to L<ping>, except it works with the bundle
protocol.
...
These exit statuses are taken from L<ping>.
