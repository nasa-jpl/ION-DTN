...
Specify the maximum number of rotated log files to retain per agent; older
files are deleted as new ones are created. Defaults to 100; 0 disables
retention (unlimited files).
...
The manager provides a text based UI as its primary interface.  The UI provides
capabilities to list, register, or delete agents.  It can view received reports
and tables, and be used to send commands (ARIs) to registered agents.
