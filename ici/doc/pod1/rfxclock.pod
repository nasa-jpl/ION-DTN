...
=item Failed handling event.

An RFX timeline event (contact or range start/stop) could not be applied
because an unrecoverable database error was encountered.  B<rfxclock>
terminates.

=item Can't do ION 1Hz updates.
