...
including the fully qualified node numbers (FQNNs) expressed by Bundle
Protocol endpoint IDs conforming to the "ipn" scheme.
...
=item int bssp_attach(void)

Attaches the application to BSSP functionality on the local computer.  Returns
0 on success, -1 on any error.

=item void bssp_detach(void)

Terminates all access to BSSP functionality on the local computer.

=item int bssp_engine_is_started(void)

Returns 1 if the local BSSP engine has been started and not yet stopped,
0 otherwise.

=item int bssp_send(uvast destinationEngineId, unsigned int clientId,
...
=item int bssp_get_notice(unsigned int clientId, BsspNoticeType *type,
BsspSessionId *sessionId, unsigned char *reasonCode, unsigned int *dataLength,
SdrObject *data)
...
=item void bssp_release_data(SdrObject data)
