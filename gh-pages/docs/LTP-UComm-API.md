...
To develop one's own underlying communication protocol implementation to support LTP, the `udplsi.c` and `udplso.c` programs are good templates to use. For a transport-agnostic skeleton with the UDP-specific scaffolding (DNS, IPv6, token-bucket pacing, `sendmmsg` batching) stripped out, plus a single-file "transport seam" you swap to retarget the link to a serial line, SpaceWire interface, FPGA FIFO, or shared-memory benchmark substrate, see the [Generic LTP Link Service Template (xlso/xlsi)](LTP-Generic-Link-Service-Template.md).
