...
control.  For all "promiscuous" prototocols - that is, protocols whose
...
induct and outduct throttles for non-promiscuous protocols (LTP, TCP)
...
=item B<b plan> I<node_ID>

The B<block plan> command.  This command disables transmission of bundles
to the indicated node and reforwards all non-critical bundles currently
queued for transmission to this node.  This may result in some or all
of these bundles being enqueued for transmission (actually just retention)
to the pseudo-node "limbo".

=item B<u plan> I<node_ID>
