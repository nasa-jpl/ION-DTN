...
The B<view transactions> command. This command lists all active CFDP transactions
(both outbound and inbound) with their current state (Active, Suspended, or
Canceled), progress (bytes transferred / total file size), and associated filenames.
This is useful for monitoring multiple concurrent file transfers.
...
For acknowledged mode, e.g., non-zero closure latency, this will lead to 
