...
int bsp_pcbCheck(AcqExtBlock *blk, AcqWorkArea *wk)
{
    ...
    bundle->payload.content = zco_add_reference(bpSdr, bprk->newBundle);
    zco_destroy_reference(bpSdr, bprk->newBundle);
    sdr_end_xn(bpSdr);
    MRELEASE(bprk);
...
as expected.", NULL);
    ...
    *(asb.resultData) = BSP_CSPARM_KEY_INFO;
    ...
    // Replace payload data with encrypted data in place, after destroying old reference
    sdr_begin_xn(bpSdr);
    zco_destroy_reference(bpSdr, bundle->payload.content);
    bundle->payload.content = zco_add_reference(bpSdr, encryptedPayloadZco);
    zco_destroy_reference(bpSdr, encryptedPayloadZco);
    sdr_end_xn(bpSdr);
    ...
    if((raw_asb == NULL) || (blk->dataLength == 0))
    {
        ...
        if(eidRefs != NULL) lyst_destroy(eidRefs);
	sdr_begin_xn(bpSdr);
	// This will deallocate all sdr memory
	zco_destroy_reference(bpSdr, encryptedPayloadZco);
	sdr_end_xn(bpSdr);
...
{ 
    ...
    // then isolate the payload in the reference.  We do this to put it back together
    // with decrypted payload later
    sdr_begin_xn(bpSdr);
    bundleRef = zco_add_reference(bpSdr, bprk->oldBundle);
    CHKERR(bundleRef);

    // grab the header & trailer
    zco_start_receiving(bpSdr, bundleRef, &reader);
    if(bprk->headerLen > 0)
    {
        bprk->headerBuff = MTAKE(bprk->headerLen);
        if(bprk->headerBuff == NULL ||
           (bytesTransmitted = zco_receive_headers(bpSdr, &reader, bprk->headerLen, (char *)bprk->headerBuff))
           != bprk->headerLen)
        {
            PCB_DEBUG_ERR("x bsp_pcbIsolatePayload: Failed to assign memory to buffer and grab full header\
                 received %d bytes instead of needed %d", bytesTransmitted, bprk->headerLen);
            sdr_end_xn(bpSdr);
            return -1;
        }
        zco_delimit_source(bpSdr, &reader, bprk->payloadLen);
    } 
    if(bprk->trailerLen > 0)
    {
        bprk->trailerBuff = MTAKE(bprk->trailerLen);
        if(bprk->trailerBuff == NULL || 
           (bytesTransmitted = zco_receive_trailers(bpSdr, &reader, bprk->trailerLen,
                                   (char *)bprk->trailerBuff)) < bprk->trailerLen)
        {
            PCB_DEBUG_ERR("x bsp_pcbIsolatePayload: Failed to assign memory to buffer and grab full trailer\n\
                 received %d bytes instead of needed %d", bytesTransmitted, bprk->trailerLen);
            sdr_end_xn(bpSdr);
            return -1;
        }
    }
                  ...
                  
{
   ...
   sdr_begin_xn(bpSdr);
   dataRef = zco_add_reference(bpSdr, payloadData);
