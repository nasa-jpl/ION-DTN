...
=item int sdr_load_profile(char *name, int configFlags, long heapWords, int memKey, char *pathName, char *restartCmd, unsigned int restartLatency)
...
predefined, fixed size.  The total amount of space occupied by an SDR in
memory and/or in a file is the sum of the size of the map plus the product
of word size and I<heapWords>.

I<memKey> is ignored if I<configFlags> does not include SDR_IN_DRAM.  It
...
memory key.  If specified, I<memKey> must be a shared memory key identifying
a pre-allocated region of shared memory whose length is equal to the total
SDR size, shared via the indicated key.
...
SDR's log file and/or database file will be written.  The name of the log
file (if any) will be "<sdrname>.sdrlog".  The name of the database file
...
=item int sdr_reload_profile(char *name, int configFlags, long heapWords, int memKey, char *pathName, char *restartCmd, unsigned int restartLatency)
...
ION.  Must B<not> be used if any database modifications were performed
...
the indicated address in the SDR database; the size of I<variable> is
...
into the database.  The size of I<variable> is used as the number of bytes
to copy.
