...
=head1 SYNOPSIS
    ...
    SdrObject sdr_table_create(Sdr sdr, int rowSize, int rowCount);
    void sdr_table_user_data_set(Sdr sdr, SdrObject table, SdrAddress userData);
    SdrAddress sdr_table_user_data(Sdr sdr, SdrObject table);
    void sdr_table_dimensions(Sdr sdr, SdrObject table, int *rowSize,
                              int *rowCount);
    void sdr_table_stage(Sdr sdr, SdrObject table);
    SdrAddress sdr_table_row(Sdr sdr, SdrObject table,
                          unsigned int rowNbr);
    int sdr_table_destroy(Sdr sdr, SdrObject table);
...
=item SdrObject sdr_table_create(Sdr sdr, int rowSize, int rowCount)
...
=item void sdr_table_user_data_set(Sdr sdr, SdrObject table,
SdrAddress userData)
...
=item SdrAddress sdr_table_user_data(Sdr sdr, SdrObject table)

Returns the value of the "user data" word of I<table>, or zero on any
error.

=item void sdr_table_dimensions(Sdr sdr, SdrObject table, int *rowSize,
int *rowCount)

Reports on the row size and row count of the indicated table, as specified
when the table was created.

=item void sdr_table_stage(Sdr sdr, SdrObject table)
...
the same as:
   ...
   sdr_stage(sdr, NULL, (SdrObject) sdr_table_row(sdr, table, 0), 0)

=item SdrAddress sdr_table_row(Sdr sdr, SdrObject table, unsigned int rowNbr)
...
=item void sdr_table_destroy(Sdr sdr, SdrObject table)
