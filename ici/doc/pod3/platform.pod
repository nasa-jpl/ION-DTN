...
=item REQUIRE(condition)

The REQUIRE macro is an "assert" mechanism.  It terminates the task, possibly
writing a core file for fault analysis, if I<condition> is false. 

=item CHECKERR(condition)

The CHECKERR macro is an "assert" mechanism.  It causes the calling function
...
values they contain.  SDNVs are used extensively in the LTP and ION libraries.

=over 4

=item void encodeSdnv(Sdnv *sdnvBuffer, unsigned long value)
...
=item int decodeSdnv(unsigned long *value, unsigned char *sdnvText)
...
=item int sm_SemUnwedge(sm_SemId semId, int interval)
...
Spawns a "smunwedge" thread that attempts to take the semaphore
identified by I<semId> and, if successful, releases the semaphore and
announces that the semaphore is released.  If the smunwedge thread fails to
announce release of the semaphore within I<interval> seconds, the semaphore
is unilaterally released by the parent thread.  Returns 1 if the
semaphore was wedged and had to be released, 0 if the semaphore was not
wedged, -1 on any error.
...
=item void sm_TaskVarAdd(int *var)

Establishes the static variable I<var> as a "task variable"
belonging to the invoking task, so that it can be referenced as an
external variable in application code; all tasks running that 
code will have their own private values for I<var> even if they
are in the same address space.  Not applicable for UNIX, since
every UNIX process already has its own private value for every
static variable.
