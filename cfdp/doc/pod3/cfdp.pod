...
to be identical to the BP (CBHE) notion of fully-qualified node number as
expressed in an ipn-scheme endpoint ID.
...
=item MetadataList cfdp_create_usrmsg_list(void)
...
=item MetadataList cfdp_create_fsreq_list(void)
...
=item int cfdp_get_event(CfdpEventType *type, time_t *time, int *reqNbr,
CfdpTransactionId *transactionId, char *sourceFileNameBuf,
char *destFileNameBuf, uvast *fileSize, MetadataList *messagesToUser,
uvast *offset, unsigned int *length, unsigned int *recordBoundsRespected,
CfdpContinuationState *continuationState, unsigned int *segMetadataLength,
char *segMetadataBuffer, CfdpCondition *condition, uvast *progress,
CfdpFileStatus *fileStatus, CfdpDeliveryCode *deliveryCode,
CfdpTransactionId *originatingTransactionId, char *statusReportBuf,
...
The closureRequested field is defined as unsigned int and is used as a
Boolean flag with the following values: 0 = No closure requested
(Unacknowledged mode), and 1 = Closure requested (Acknowledged mode).
Applications can use this field to determine whether a transaction is
operating in Acknowledged or Unacknowledged mode.

On error, returns zero but sets errno to EINVAL.  Returns -1 on
system failure, zero otherwise.

=item void cfdp_interrupt(void)
...
request is subject to the configuration values in I<dirListTask>.  This
function is now a thin wrapper around B<cfdp_rls_extended> with
I<listingOptions> set to zero, producing the legacy listing format on
the wire (a stream of NUL-terminated entry names).

=item int cfdp_rls_extended(CfdpNumber *respondentEntityNbr, unsigned int utParmsLength, unsigned char *utParms, char *sourceFileName, char *destFileName, CfdpReaderFn readerFn, CfdpHandler *faultHandlers, unsigned int flowLabelLength, unsigned char *flowLabel, unsigned int closureLatency, MetadataList messagesToUser, MetadataList filestoreRequests, CfdpDirListTask *dirListTask, unsigned int listingOptions, CfdpTransactionId *transactionId)

Same as B<cfdp_rls> except that I<listingOptions> may be used to request
extended directory listing behavior.  C<CFDP_DIRLIST_OPTION_STATUS_BYTES>
causes the responder to produce a v2 manifest in which each entry is
prefixed with a one-byte status flag and a one-byte type indicator (one
of C<CFDP_DIRENT_REGULAR>, C<CFDP_DIRENT_DIRECTORY>,
C<CFDP_DIRENT_SYMLINK>, C<CFDP_DIRENT_OTHER>, or
C<CFDP_DIRENT_UNKNOWN>), and the response carries an "incomplete" flag
indicating whether the responder dropped any entries.

C<CFDP_DIRLIST_OPTION_RECURSIVE> requests a manifest for the subtree
rooted at I<dirListTask-E<gt>directoryName>.  Directory entries in a
recursive manifest are identified by a trailing C</>.  The status-byte
and recursive options may be combined.

Extended requests are wire-compatible with old responders: an old responder
ignores the trailing options byte and replies with a legacy (type 17)
response, so a new requestor that calls B<cfdp_rls_extended> against an
old responder gracefully sees a legacy manifest.

The status byte currently carries the C<CFDP_DIRENT_NAME_TRUNCATED>
bit (reserved -- the responder drops over-length entries rather than
truncating, so this bit is not produced today, but parsers should
honor it for forward compatibility).  Other status bits are reserved
for future use.
