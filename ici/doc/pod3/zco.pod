...
=item int zco_enough_file_space(Sdr sdr, unsigned int length)

Returns 1 if the total remaining file system space available for ZCOs is
greater than I<length>.  Returns 0 otherwise.

=item int zco_enough_heap_space(Sdr sdr, unsigned int length)
...
=item void zco_start_transmitting(Sdr sdr, Object zco, ZcoReader *reader)
...
=item void zco_start_receiving(Sdr sdr, Object zco, ZcoReader *reader)
