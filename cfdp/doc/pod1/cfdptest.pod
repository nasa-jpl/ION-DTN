...
The B<closure control> command.  This command establishes the transaction
closure mode for all subsequent file transmission operations.  When it is
set to zero, the file transmission is "open loop" and the CFDP transaction
at the sending entity finishes when the EOF is sent.  When it set to 1, a
"Finished" PDU will be sent back to the sending CFDP entity when the
transaction finishes at the receiving entity; the transaction finishes at
the sending entity only when that "Finished" PDU is received.
