...
the metadata header.  The program's default behavior auto-increments file
...
available with the ITAR controlled ION NASA Baseline).

The maximum receivable file content size is limited to 10% of the ION
working memory (wmSize).  Files exceeding this limit will be rejected.
Additional runtime constraints may further reduce the effective limit.
