...
int 	bss_setCtDueTimer(Bundle bundle, Object bundleAddr)
{
	...
	{
		elt = sdr_list_first(sdr, bundle.xmitRefs);
		if (elt == 0)
		{
			putErrmsg("XmitRefs stack is empty.", NULL);
			return -1;	
		}

		xrAddr = sdr_list_data(sdr, elt);
		if (xrAddr == 0)
		{
			putErrmsg("Could not retrieve elt data", NULL);
			return -1;
		}

		GET_OBJ_POINTER(sdr, XmitRef, xr, xrAddr);

		/* 
		 *  Checking whether bundle is enqueued to limbo queue. In
		 *  that case do nothing. Otherwise, set the custody      
		 *  expiration event for this particular bundle according
		 *  to BSS configuration.					      
		 */

		if (sdr_list_list(sdr, xr->ductXmitElt) ==
				(getBpConstants())->limboQueue)
		{
			return 0;	/*	Bundle is in limbo.	*/
		}

		sdr_string_read(sdr, proxNodeEid, xr->proxNodeEid);	
	...
	/*	bundle.xmitRefs list is empty or an invalid RTT value
	 *	was returned.						*/	
