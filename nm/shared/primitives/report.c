...
rpt_t*   rpt_create(time_t time, Lyst entries, eid_t recipient)
{
	...
	DTNMP_DEBUG_ENTRY("rpt_create","(%d,"UVAST_FIELDSPEC",%s)",
			          time, (uvast) entries, recipient.name);

	/* Step 1: Allocate the message. */
	if((result = (rpt_t*) MTAKE(sizeof(rpt_t))) == NULL)
	{
...
void  rpt_clear_lyst(Lyst *list, ResourceLock *mutex, int destroy)
{
    ...
    DTNMP_DEBUG_ENTRY("rpt_clear_lyst","("UVAST_FIELDSPEC","UVAST_FIELDSPEC", %d)",
...
rpt_t*  rpt_deserialize_data(uint8_t *cursor, uint32_t size, uint32_t *bytes_used)
{
	...
	DTNMP_DEBUG_ENTRY("rpt_deserialize","("UVAST_FIELDSPEC", %d,"UVAST_FIELDSPEC")",
	...
	*bytes_used = 0;
	...
	if((rx = (char *) MTAKE(rx_len + 1)) == NULL)
	{
	...
	DTNMP_DEBUG_EXIT("rpt_deserialize_data","->"UVAST_FIELDSPEC,(uvast)result);
	return result;
}
...
uint8_t* rpt_serialize(rpt_t *rpt, uint32_t *len)
{
	...
	DTNMP_DEBUG_ENTRY("rpt_serialize","("UVAST_FIELDSPEC","UVAST_FIELDSPEC")",
			          (uvast)rpt, (uvast) len);
	...
	*len = time.length + num_entries_sdnv.length + entries_len + rx_sdnv.length + strlen(rpt->recipient.name);
	...
	DTNMP_DEBUG_EXIT("rpt_serialize","->"UVAST_FIELDSPEC,(uvast)result);
	return result;
}
...
void rpt_entry_clear_lyst(Lyst *list, ResourceLock *mutex, int destroy)
{
    ...
    DTNMP_DEBUG_ENTRY("rpt_entry_clear_lyst","("UVAST_FIELDSPEC","UVAST_FIELDSPEC", %d)",
			          (uvast) list, (uvast) mutex, destroy);
...
char *rpt_entry_lst_to_str(Lyst entries)
{
	...
	temp_space = (char **) MTAKE(num_entries * sizeof(char *));
	temp_len   = (uint32_t*) MTAKE(num_entries * sizeof(uint32_t));
...
char *rpt_entry_to_str(rpt_entry_t *entry)
{
	...
	if((result = (char *) MTAKE(overhead+mid_len+tdc_len)) == NULL)
	{
