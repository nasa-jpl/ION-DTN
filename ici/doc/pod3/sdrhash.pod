...
=head1 SYNOPSIS
    ...
    SdrObject sdr_hash_create(Sdr sdr, int keyLength,
                           int estNbrOfEntries,
                           int meanSearchLength);
    int sdr_hash_insert(Sdr sdr, SdrObject hash, char *key,
                        SdrAddress value, SdrObject *entry);
    int sdr_hash_delete_entry(Sdr sdr, SdrObject entry);
    SdrAddress sdr_hash_entry_value(Sdr sdr, SdrObject hash, SdrObject entry);
    int sdr_hash_retrieve(Sdr sdr, SdrObject hash, char *key,
                          SdrAddress *value, SdrObject *entry);
    int sdr_hash_count(Sdr sdr, SdrObject hash);
    int sdr_hash_revise(Sdr sdr, SdrObject hash, char *key,
                        SdrAddress value);
    int sdr_hash_remove(Sdr sdr, SdrObject hash, char *key,
                        SdrAddress *value);
    int sdr_hash_destroy(Sdr sdr, SdrObject hash);
...
=item SdrObject sdr_hash_create(Sdr sdr, int keyLength, int estNbrOfEntries,
int meanSearchLength)

Creates an SDR hash table.  Returns the SDR address of the new hash table
on success, zero on any error.

=item int sdr_hash_insert(Sdr sdr, SdrObject hash, char *key, SdrAddress value,
SdrObject *entry)
...
=item int sdr_hash_delete_entry(Sdr sdr, SdrObject entry)

Deletes the hash table entry identified by I<entry>.  Returns zero on
success, -1 on any error.

=item SdrAddress sdr_hash_entry_value(Sdr sdr, SdrObject hash, SdrObject entry)

Returns the value of the hash table entry identified by I<entry>.

=item int sdr_hash_retrieve(Sdr sdr, SdrObject hash, char *key,
SdrAddress *value, SdrObject *entry)
...
=item int sdr_hash_count(Sdr sdr, SdrObject hash)

Returns the number of entries in the hash table identified by I<hash>.

=item int sdr_hash_revise(Sdr sdr, SdrObject hash, char *key, SdrAddress value)
...
=item int sdr_hash_remove(Sdr sdr, SdrObject hash, char *key, SdrAddress *value)
...
=item void sdr_hash_destroy(Sdr sdr, SdrObject hash)
