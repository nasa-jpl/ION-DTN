...
=item int zco_get_aggregate_length(Sdr sdr, Object location, vast offset, vast length, vast *fileSpaceOccupied, vast *heapSpaceOccupied)

Populates I<*fileSpaceOccupied> and I<*heapSpaceOccupied> with the total
number of ZCO space bytes occupied by the extents of the zco at I<location>,
from I<offset> to I<offset + length>.  If I<offset> isn't the start of an
extent or I<offset + length> isn't the end of an extent, returns -1 in both
fields.

=item Object zco_create(Sdr sdr, ZcoMedium firstExtentSourceMedium, Object firstExtentLocation, vast firstExtentOffset, vast firstExtentLength, ZcoAcct acct, unsigned char provisional)
...
reference object, SDR heap object, or ZCO, depending on the value of
I<firstExtentSourceMedium>, and (b) I<firstExtentOffset> indicates how manyx
leading bytes of the source data object should be skipped over when adding
the initial source data extent to the new ZCO.  A negative value for
...
is the additive inverse of this value.  A non-zero value for I<provisional>
indicates that this ZCO will occupy non-Restricted Inbound ZCO space.  This
space is a critical resource, so provisional ZCOs are subject to defensive
destruction if they cannot immediately be migrated into the Outbound ZCO
space pool.  On success, returns the SDR location of the new ZCO.  Returns
...
=item int zco_is_provisional(Sdr sdr, Object zco)

Returns an indicator as to whether or not I<zco> is flagged as "provisional".
