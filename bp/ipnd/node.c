 ...
 *                                 from this socket.
 * @return     Socket on succses, -1 on error.
 */
...
for beacon sending socket.",
	...
	if (enabledBroadcastSending)
	{
		...
		{
			putSysErrmsg("send-thread: Can't set broadcast sending \
option", NULL);
		}
	}
...
beacon (%dB) to %s:%d.", rawBeaconLength, dest->addr.ip, dest->addr.port);
	...
	isprintf(buffer, sizeof buffer,
	         "[i] send-thread: Beacon (%dB) sent to %s:%d correctly: ",
	         rawBeaconLength, dest->addr.ip, dest->addr.port);
	printText(buffer);
	logBeacon(beacon);
	MRELEASE(rawBeacon);
	return 0;
...
void	*sendBeacons(void *attr)
{
	...
	while (1)
	{
		...
		if (result < 0)
		{
			putErrmsg("send-thread: "
			          "Error waiting for destinations lyst.", NULL);
			return (void *) - 1;
		}

		llcv_lock(ctx->destinationsCV);
		nDestinationElt = lyst_first(destinations);
		nDestination = lyst_data(nDestinationElt);
		...
		isprintf(buffer, sizeof buffer,
		         "[i] send-thread: Started beacon sending to %s:%d.",
		         nDestination->addr.ip, nDestination->addr.port);
		printText(buffer);
		...
		{
			printText("[i] send-thread: Previously sent beacon \
not found.");
			...
			{
				putErrmsg("send-thread: Can't init beacon.",
						NULL);
				llcv_unlock(ctx->destinationsCV);
				continue;
			}
		...
		else
		{
			...
			if (beaconHasChanged)
			{
				...
				/* If beacon has changed, create a new
				 * beacon. 				*/
...
beacon.", NULL);
				...
				/* Set old sequence number to new
				 * beacon and send it.			*/
			...
			else
			{
					...
					(nDestination->beacon.canonicalEid))
				{
					isprintf(buffer, sizeof buffer,
					 	...
					 	"and node does not have an "
					 	"active connection with %s:%d.",
				         	nDestination->addr.ip,
					 	nDestination->addr.port);
					printText(buffer);

					/*  If beacon has not changed
					 *  and we don't have an active
					 *  connection with this
					 *  destination, just update
					 *  the sequence number and
					 *  send the old beacon again.	*/

					oldBeacon->sequenceNumber++;
					if (sendBeacon(oldBeacon, nDestination,
							sendSocket) < 0)
					{
						putErrmsg("send-thread: Can't \
send beacon.", NULL);
						llcv_unlock
							(ctx->destinationsCV);
						continue;
...
socket.", NULL);
		...
		if (getIpv4AddressType(listenAddr->ip) == UNICAST)
		{
			...
			if (joinMulticastGroups(listenAddresses, listenSocket,
					listenAddr->ip) < 0)
			{
				putErrmsg("receive-thread: Eror joining \
...
static void bp_discover_contact(char acquired, IPNDCtx* ctx, char* eid)
{
	...
	Destination		*dest;
	int			xmitRate = 1000000; // TODO
	int			recvRate = 1000000; // TODO
	...
	for (curD = lyst_first(ctx->destinations); curD != NULL; curD = nextD)
	{
		...
		socketSpec[0] = '\0';
		for (cur = lyst_first(dest->beacon.services); cur != NULL;
				cur = next)
		{
			next = lyst_next(cur);
			...
			if (def->number == 64)
			{
				...
				{
					isprintf(socketSpec, sizeof socketSpec,
						"%d.%d.%d.%d:%d", 
						(unsigned char) def->data[3],
						(unsigned char) def->data[4],
						(unsigned char) def->data[5],
						(unsigned char) def->data[6],
						((unsigned char) def->data[8])
							* 256 + 
						(unsigned char) def->data[9]);
				}
		...
		if (socketSpec[0])
		{
			if (acquired)
			{
				isprintf(buffer, sizeof buffer,
					"[i] receive-thread: "
					"bp_discover_contact_acquired(%s, %s, \
%s, %d, %d)", socketSpec, eid, claProtocol, xmitRate, recvRate);
			...
			else
			{
				isprintf(buffer, sizeof buffer,
					"[i] receive-thread: "
					"bp_discover_contact_lost(%s, %s, %s)",
					socketSpec, eid, claProtocol);
...
void *receiveBeacons(void *attr)
{
	...
	listenSockets = setUpListenSockets(ctx->listenAddresses,
	                                   ctx->enabledBroadcastReceiving,
	                                   &numListenSockets);
	unlockResource(&ctx->configurationLock);

	if (numListenSockets == 0 || listenSockets == NULL)
	{
		putErrmsg("receive-thread: "
		          "No listen sockets configured. IPND will not receive "
		          "any beacon.", NULL);
		return (void *) - 1;
	}
	...
	/* Make sure memory is released when the thread is cancelled. */
	...
	while (1)
	{
		...
		for (i = 0; i < FD_SETSIZE; ++i)
		{
			...
			if ((recvDataBufferLen = irecvfrom(recevingSocket,
					recvDataBuffer, MAX_BEACON_SIZE, 0,
...
to string.", NULL);
			...
			/* Check if we have recv data from one of our
			 * unicast addresses. 				*/
			...
			/* Check if recv data is from a known neighbor */
			...
			if (nbElt != NULL)
			{
				...
				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "Sender %s:%d is a known neighbor.",
				         srcAddrStr, srcAddrPort);
				printText(buffer);
			}
			else
			{
				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "Sender %s:%d is an unknown neighbor.",
				         srcAddrStr, srcAddrPort);
				printText(buffer);
			}
			...
			if (*recvDataBuffer == IPND_VERSION2)
			{
				printText("[i] receive-thread: "
				          "Received IPND version 2 beacon.");
				continue;
			}
			else if (*recvDataBuffer == IPND_VERSION4)
			{
				/* Deserialize beacon */

				if (deserializeBeacon(recvDataBuffer,
					recvDataBufferLen, &recvBeacon) < 0)
				{
					putErrmsg("receive-thread: "
					          "Error deserializing beacon.",
						  NULL);
					continue;
				}
				...
				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "Received beacon (%dB) contents: ",
				         recvDataBufferLen);
				printText(buffer);
				logBeacon(&recvBeacon);

				/* Set configured beacon period for this
				 * source address in case beacon did not
				 * include a period. 			*/
				...
				/* If it is a new neighbor add it to
				 * neighbors list.			*/
				...
				if (nb == NULL)
				{
					// update NBF
					lockResource(&ctx->configurationLock);
					updateCtxNbf(recvBeacon.canonicalEid,
							strlen(recvBeacon.
								canonicalEid));
					unlockResource(&ctx->configurationLock);
					...
					lockResource(&ctx->neighborsLock);
					nb = (IpndNeighbor *)
						MTAKE(sizeof(IpndNeighbor));
					istrcpy(nb->addr.ip, srcAddrStr,
						INET_ADDRSTRLEN);
					nb->addr.port = srcAddrPort;
					lyst_insert(ctx->neighbors, (void *)nb);
					unlockResource(&ctx->neighborsLock);

					isprintf(buffer, sizeof buffer,
					         "[i] receive-thread: "
					         "Sender %s:%d added as new \
neighbor.", srcAddrStr, srcAddrPort);
					printText(buffer);
					newNb = 1;
				}
				else
				...
				// create bidirectional link if our eid
				// is present

				if (bloom_check(&recvBeacon.bloom, ctx->srcEid,
						strlen(ctx->srcEid)) == 1)
				{
					nb->link.bidirectional = 1;
					isprintf(buffer, sizeof buffer,
					         "[i] receive-thread: "
					         "Sender's %s:%d NBF contains \
our EID - bidirectional.", srcAddrStr, srcAddrPort);
					printText(buffer);
				}
			...
			llcv_lock(ctx->destinationsCV);
			if (findDestinationByAddr(&nb->addr, ctx->destinations)
					== NULL)
			{
				newDest = (Destination *)
...
new destination.", NULL);
				...
				lyst_insert(ctx->destinations, (void *)newDest);

				copyBeacon(&newDest->beacon, &nb->beacon);

				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "Sender %s:%d added, new destination.",
				         srcAddrStr, srcAddrPort);
				printText(buffer);
			}
			...
			{
				bp_discover_contact(1,
						ctx, nb->beacon.canonicalEid);
				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "CLA told that link %s:%d is up.",
				         srcAddrStr, srcAddrPort);
				printText(buffer);
			}
			...
			else if (nb->link.state == DOWN)
			{
				bp_discover_contact(0, ctx,
						nb->beacon.canonicalEid);
				isprintf(buffer, sizeof buffer,
				         "[i] receive-thread: "
				         "CLA told that link %s:%d is down.",
				         srcAddrStr, srcAddrPort);
				printText(buffer);
			}
		}
...
void *expireNeighbors(void *attr)
{
	...
	while (1)
	{
		sleep(MAX(MAX(ctx->announcePeriods[UNICAST],
		...
		for (i = 0; i < lyst_length(ctx->neighbors); i++)
		{
			...
			if (time(NULL) < nb->beaconReceptionTime
					+ nb->beacon.period * 2)
				/* Neighbor still not expired */
				continue;

			isprintf(buffer, sizeof buffer,
			         "[i] expire-thread: "
			         "Beacon not recv in %d seconds. "
			         "Neighbor %s:%d has disappeared",
