...
This is the SVR4 IPC shared-memory key used to attach this node's SDR
working memory.  The default (SM_NO_KEY, equivalently 0) selects the
legacy host-global key SDR_SM_KEY (255 * 256 = 65280 = 0xFF00), which is
shared by every ION node on the host.  Set sdrWmKey to a unique value in
each node's ionconfig when running multiple ION nodes on the same host
and you want each node's SDR working memory to be independent (so that
shutting down one node with C<ionexit n> does not destroy SDR working
memory still in use by the others).  See the ION Shutdown Guide for
the full multi-node shutdown ordering implications.
...
SDR_IN_DRAM | SDR_REVERSIBLE | SDR_BOUNDED).  The SDR configuration flags are
...
When this bit is set, a transaction that is cancelled after modifying the
heap is rolled back from that log.  On a running node the rollback is then
followed by a "reversal cleanup" step that reloads working memory, which ION
hardwires to the ionrestart(1) utility -- so a reversal on a live node
restarts the node.  That cleanup command is fixed when the node is
initialized and is B<not> configurable through this file; the only control
this file offers is this bit itself.  Clearing the bit disables reversibility
(and therefore the automatic restart), but then a transaction that is
cancelled after modifying the heap can no longer be backed out and instead
raises an unrecoverable SDR error.  See ionrestart(1) for the restart
behavior and how to suppress it.
...
reversible.  The default value is B</tmp>.
...
I<logKey> is specified, a block of shared memory of this size will be allocated
...
This is the size (in bytes) of the shared memory segment to allocate for
PSM partition activity tracing (sptrace).  When B<psmwatch> starts a trace
episode, it uses this value instead of its built-in default of 20000000
(20 million bytes).  Larger values allow longer trace sessions before trace
memory is exhausted.  If zero (the default), B<psmwatch> uses 20000000.
