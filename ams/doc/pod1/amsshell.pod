...
continuum and a registrar for the root unit of the indicated venture (which
may both be instantiated in a single B<amsd> daemon task) must be running
in order for B<amsshell> to run.
