...
B<amslog> I<application_name> I<authority_name> I<[{ s | i }]>
...
root unit of the venture identified by I<application_name> and
I<authority_name>.  A configuration server for the local continuum and
a registrar for the root unit of the indicated venture (which may both
be instantiated in a single B<amsd> daemon task) must be running in order
for B<amslog> to run.
