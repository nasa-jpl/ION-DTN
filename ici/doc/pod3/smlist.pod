...
=item PsmAddress sm_list_create_unlocked(PsmPartition partition)

Same as sm_list_create(), except that the new list is created without a
list-level mutex semaphore; sm_list operations on it perform no locking.
This variant exists to conserve semaphores (see platform(3)) on nodes that
create very large numbers of lists.  It is safe to use B<only> when the
caller guarantees that all access to the list is serialized by some other
means, such as a surrounding SDR transaction; otherwise the list is not
protected against concurrent modification.  Returns the PsmAddress of the
list, or 0 on any error.
...
=item int sm_list_clear(PsmPartition partition, PsmAddress list,
SmListDeleteFn delete, void *argument)
...
=item int sm_list_destroy(PsmPartition partition, PsmAddress list,
SmListDeleteFn delete, void *argument)

Destroy a list.  Same as sm_list_clear(), but additionally frees the list
structure itself.  Returns 0 on success, -1 on any error.

=item int sm_list_user_data_set(PsmPartition partition, PsmAddress list,
PsmAddress userData)
...
=item size_t sm_list_length(PsmPartition partition, PsmAddress list)

Return the number of elements in the list.

=item PsmAddress sm_list_insert(PsmPartition partition, PsmAddress list,
PsmAddress data, SmListCompareFn compare, void *dataBuffer)
...
the data value of the new element (in I<dataBuffer>) according to the
collating sequence established by I<compare>.  Returns the PsmAddress of
the new element, or 0 on any error.

=item PsmAddress sm_list_insert_first(PsmPartition partition, PsmAddress list,
PsmAddress data)

=item PsmAddress sm_list_insert_last(PsmPartition partition, PsmAddress list,
PsmAddress data)
...
=item PsmAddress sm_list_insert_before(PsmPartition partition, PsmAddress elt,
PsmAddress data)

=item PsmAddress sm_list_insert_after(PsmPartition partition, PsmAddress elt,
PsmAddress data)
...
=item int sm_list_delete(PsmPartition partition, PsmAddress elt,
SmListDeleteFn delete, void *argument)
...
passed the PsmAddress of the list element and the I<argument>
...
=item PsmAddress sm_list_search(PsmPartition partition, PsmAddress elt,
SmListCompareFn compare, void *dataBuffer)

Search a list for an element whose data matches the data in I<dataBuffer>.  If
...
located whose data value is equal to ((PsmAddress) I<dataBuffer>).
...
=item PsmAddress sm_list_data_set(PsmPartition partition, PsmAddress elt,
PsmAddress data)
...
=item int sm_list_header_sane(PsmPartition partition, PsmAddress list)

Return 1 if the list header identified by I<list> is structurally sane, 0
otherwise.  The header is considered sane if its lock field is either
SM_SEM_NONE (an unlocked list) or a valid semaphore index.  This function
does not take the list's lock and does not assert; it merely inspects the
header in place.  It is intended as a use-after-free guard for a I<list>
handle whose PsmAddress still round-trips through psa()/psp() but whose
backing memory may have been freed and recycled, in which case the lock
field holds whatever bytes the recycler wrote.  Returns 0 if I<partition>
is NULL or I<list> is 0.
...
memory) and an argument, I<dataBuffer>, which is nominally the address
...
list element (I<elt> and an argument, I<argData>, which if non-NULL is
