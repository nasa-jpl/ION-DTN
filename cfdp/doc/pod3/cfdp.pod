...
the v2 directory listing format.  Currently the only defined option bit
is C<CFDP_DIRLIST_OPTION_STATUS_BYTES>; when set, the responder produces
a manifest in which each entry is prefixed with a one-byte status flag
and a one-byte type indicator (one of C<CFDP_DIRENT_REGULAR>,
C<CFDP_DIRENT_DIRECTORY>, C<CFDP_DIRENT_SYMLINK>, C<CFDP_DIRENT_OTHER>,
or C<CFDP_DIRENT_UNKNOWN>), and the response carries an "incomplete"
flag indicating whether the responder dropped any entries.

The v2 request is wire-compatible with old responders: an old responder
