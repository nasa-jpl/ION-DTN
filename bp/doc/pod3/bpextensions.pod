...
-- are defined in the I<bpP.h> header file.  In each case, the extension
...
also as defined in the I<bpP.h> header file.  Each ExtensionDef must supply:
...
before (0) or after (1) the payload block when new bundles are locally sourced.

A pointer to an B<offer> function.

A pointer to a B<release> function.

A pointer to an B<accept> function.
...
structures named I<extensions>.  The order of appearance of extension
definitions in the extensions array determines the order in which
...
extensions array, referencing those declared functions.

5.  Develop the implementations of those functions in one or more new source
code files.

6.  Add the object file or files for the new extension implementation source
...
from the serialized bundle.)  This function is automatically called
