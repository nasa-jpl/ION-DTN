...
B<bibeclo> I<peer_EID>

=head1 DESCRIPTION

B<bibeclo> is a background "daemon" task that extracts bundles from the
queues of bundles that are ready for transmission via bundle-in-bundle
encapsulation (BIBE) to I<peer_EID>, encapsulates them in BP administrative
records of (non-standard) record type 7 (BP_BIBE_PDU), and sends those
administrative records in encapsulating bundles destined for I<peer_EID>.  The
forwarding of BPDUs for which custodial acknowledgment is configured
causes B<bibeclo> to post
...
No BIBE outduct with duct name I<peer_EID> exists.  Use B<bpadmin>
