...
#if CGRDEBUG
printf("\nSeeking a class %d route via contact to node %lu arrival time %lu.\n",
payloadClass, rootContact->toNode, rootWork->arrivalTime);
#endif
	current = rootContact;
...
#if CGRDEBUG
printf("\nConsidering contacts from node %lu.\n", arg.fromNode);
#endif
...
contact->fromNode, contact->toNode, contact->fromTime);
			...
			{
#if CGRDEBUG
printf("Contact is not from the node we're routing from (%lu).\n",
arg.fromNode);
#endif
...
printf("Dijkstra search has ended.\n");
		...
		current = nextContact;
#if CGRDEBUG
printf("Now continuing from node %lu.\n", current->toNode);
#endif
			...
			int payloadClass, PsmAddress *routeAddr)
{
	...
	*routeAddr = 0;		/*	Default.			*/
	...
	{
#if CGRDEBUG
printf("----No more class-%d routes to node %lu.\n", payloadClass,
stationNode->nodeNbr);
#endif
	...
	{
#if CGRDEBUG
printf("----Found class %d route via node %lu: start @ %lu, deliver @ %lu.\n",
payloadClass, route->toNodeNbr, route->fromTime, route->deliveryTime);
#endif
...
#if CGRDEBUG
printf("<<<Computed all routes from node %lu to node %lu.\n\n", getOwnNodeNbr(),
stationNode->nodeNbr);
#endif
...
#if CGRDEBUG
printf("Trying class-%d route via node %lu: start @ %lu, deliver @ %lu.\n",
route->payloadClass, route->toNodeNbr, route->fromTime, route->deliveryTime);
#endif
...
route->toNodeNbr, hopCount, route->deliveryTime);
	...
	proxNode->forfeitTime = route->toTime;
#if CGRDEBUG
printf("Proposing proximate node %lu: %d hops, delivery at %lu.\n",
route->toNodeNbr, hopCount, route->deliveryTime);
#endif
	return 0;
}
...
stationNode->nodeNbr, deadline);
	...
	for (elt = sm_list_first(ionwm, routes); elt; elt = nextElt)
	{
		...
		route = (CgrRoute *) psp(ionwm, addr);
#if CGRDEBUG
printf("Checking class-%d route via node %lu: start @ %lu, deliver @ %lu.\n",
route->payloadClass, route->toNodeNbr, route->fromTime, route->deliveryTime);
#endif
...
#if CGRDEBUG
printf("\n\n\nCGR looking for a route to node %lu for bundle of length %lu.\n",
