...
// 11-06-2020 Jay Gao debugging android crash add logcat
#include <android/log.h>
#define APPNAME "IonDTN: smrbt.c"
				...
				SmRbtNode **buffer)
{
	...
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 367");
	...
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 377");

	nodePtr = (SmRbtNode *) psp(partition, node);

	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 382");
	nodePtr->rbt = rbt;
	...
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 390");
	
	if (buffer)	/*	Tree already has a root.		*/
	{
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 395");
		nodePtr->isRed = 1;
		*buffer = nodePtr;
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 399");
	}
	else		/*	This is the first node in the tree.	*/
	{
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 404");
		nodePtr->isRed = 0;	/*	Root is always black.	*/
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "createNode() @ line 407");
	}
			...
			SmRbtCompareFn compare, void *dataBuffer)
{
	...
	{
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert @ line 447");
		node = createNode(file, line, partition, rbt, 0, data, NULL);
		if (node)
		{
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 452");
			rbtPtr->root = node;
			rbtPtr->length = 1;
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 456");
		}

		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 460");
		unlockSmrbt(rbtPtr);
		return node;
	}
	...
	while (1)
	{
		if (node == 0)	/*	(Can't be tree's root node.)	*/
		{
			...
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 485");
			node = createNode(file, line, partition, rbt, parent,
					data, &nodePtr);
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 489");
			if (node == 0)
			{
				// 11-06-2020 Jay Gao debugging android crash add logcat
				__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 493");
				unlockSmrbt(rbtPtr);
				return 0;
			}
		...
		{
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 508");
			nodePtr = (SmRbtNode *) psp(partition, node);
		...
		{
			// 11-06-2020 Jay Gao debugging android crash add logcat
			__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 540");
		...
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 567");
		...
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 575");
		...
		// 11-06-2020 Jay Gao debugging android crash add logcat
		__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 588");
		if (grandparent)
		{
	...
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 602");
	rbtPtr->length += 1;

	/*	Record the new root node object.			*/
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 607");
	rbtPtr->root = dummyRootBuffer.child[RIGHT];

	/*	Make sure the root node is black.			*/
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 612");
	nodePtr = (SmRbtNode *) psp(partition, rbtPtr->root);
	nodePtr->isRed = 0;
	unlockSmrbt(rbtPtr);
	// 11-06-2020 Jay Gao debugging android crash add logcat
	__android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Sm_rbt_insert() @ line 617");
	return node;
}
