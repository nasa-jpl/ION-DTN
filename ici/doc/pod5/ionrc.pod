...
network.  It also configures ION's data store (SDR) and shared working-memory
region using either the settings found in the I<ion_config_filename> file
or, if '' is supplied as the I<ion_config_filename>, a set of default
settings.  Please see ionconfig(5) for details.
...
=item B<a contact> I<start_time> I<stop_time> I<source_node> I<dest_node> I<xmit_data_rate>
...
=item B<m production> I<asserted_data_production_rate>

This management command sets ION's limit on the mean rate of continuous
data origination by local BP applications.  For nodes that function only
as routers this variable will normally be zero.  A value of -1, which is the
default, indicates that production is limited only by resource availability;
in that case local data production and consumption are not considered in
the computation of congestion forecasts.
...
as routers this variable will normally be zero.  A value of -1, which is the
default, indicates that the rate of local data consumption is unknown.

=item B<m occupancy> I<heap_occupancy_limit> [I<file_system_occupancy_limit>]
...
signifies "leave unchanged".  The default heap limit is 60% of the SDR data
...
This management command simply prints ION's current data store occupancy
