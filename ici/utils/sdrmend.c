		...
		int a6, int a7, int a8, int a9, int a10)
{
	...
	unsigned int	restartLatency = a7;
...
int	main(int argc, char **argv)
{
	...
	unsigned int	restartLatency = 0;

	if (argc < 6)
	{
		PUTS("Usage: sdrmend <sdr name> <config flags> <heap words> \
<heap key, e.g. -1> <pathName> [<restartCmd> <restartLatency>");
		return 0;
	}
	...
	{
		restartCmd = argv[6];
		restartLatency = strtoul(argv[7], NULL, 0);
	}
#endif
	...
	if (sdr_reload_profile(sdrName, configFlags, heapWords, heapKey,
			pathName, restartCmd, restartLatency) < 0)
	{
