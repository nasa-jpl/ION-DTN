...
the text of the command that is used to spawn the task may be provided
as a parameter to the 's' command if not using the 'seat' command.
The link service input task is terminated by B<ltpadmin> in response
