...
if [ "$#" -ne 1 ]; then
	echo "usage: $0 /absolute/path/to/ion-install-prefix" >&2
	exit 2
fi
...
cleanup()
{
	(
		...
		env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
			...
			bpadmin stop.bprc >/dev/null 2>&1 || true
		env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
			PATH="$PREFIX/bin:$PATH" \
			LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
			ionadmin stop.ionrc >/dev/null 2>&1 || true
		env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
			PATH="$PREFIX/bin:$PATH" \
			LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
			ionexit >/dev/null 2>&1 || true
	)
...
run_case()
{
	...
	env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
		PATH="$PREFIX/bin:$PATH" \
		LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
		ionadmin node.ionrc >/dev/null
	env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
		PATH="$PREFIX/bin:$PATH" \
		LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
		ionadmin "$plan.ionrc" >/dev/null
	env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
		PATH="$PREFIX/bin:$PATH" \
		LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
		bpadmin node.bprc >/dev/null
	sleep 2

	env ION_NODE_LIST_DIR="$NODE_LIST_DIR" \
		PATH="$PREFIX/bin:$PATH" \
		LD_LIBRARY_PATH="$PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
		cgrfetch -j -d udp:127.0.0.1:4556 4 2>"$trace"
