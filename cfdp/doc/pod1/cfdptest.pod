...
requests operates on the B<remote> destination.
...
Ensures the specified file is deleted but okay if it does not exist.

=item 8 = deny directory

B<Syntax:> r 8 <directory_name>

Ensures the specified directory is removed but okay if it does not exist.
...
For ION 4.1.4 or higher, B<cfdptest> will spawn a CFDP event handling thread 
for both interactive and scripted mode, this event thread receives CFDP service 
indications and simply prints lines of text to stdout to announce them.
...
cfdpadmin(1), cfdp(3), bpcp(1), bpcpd(1)