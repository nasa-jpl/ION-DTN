...
The B<region> command.  This is used to identify the region corresponding
to the contact plan that will include all contacts added until this execution
of ionadmin terminates, or until the next B<region> command is processed.
...
which consequently share a common contact plan.  Regions are notionally quite
small sets (on the order of 16-32 nodes) because contact graph routing is
...
Five different types of contact can be intermixed within an .ionrc file that
...
When both I<start_time> and I<stop_time> are zero, I<confidence> is 1.0,
and I<source_node> and I<dest_node> are identical, the contact signifies
the "registration" of this node in the region corresponding to the contact
plan of which this contact is a part.  A registration contact simply affirms
the source node's permanent membership in this region, persisting even during
periods when the node is able neither to send nor to receive data.  When
inserted into the contact plan, the contact's start and stop times are both
automatically changed from zero to the maximum POSIX time and its data rate
is set to zero.

=item 2  latent

When both I<start_time> and I<stop_time> are zero, I<confidence> is 0.0, and
I<source_node> or I<dest_node> (but not both) identify the local node, the
contact is "latent".  A latent contact is an opportunity to transmit data
either to or from the local node that is expected to occur at some time in
the future, under unknown conditions.  When the indicated communication
opportunity subsequently occurs, the latent contact is automatically
transformed into a "discovered" contact, simulating the addition of a
new discovered contact as discussed below.  Latent contacts are included
in the computation of routes to destination nodes, but a route that
commences with a latent (rather than discovered) contact cannot be
selected for the forwarding of data.  When inserted into the contact plan,
the contact's stop time is automatically changed from zero to the maximum
POSIX time and its data rate is set to zero.

=item 3  discovered

When both I<start_time> and I<stop_time> are zero, I<confidence> is 1.0, and
I<source_node> or I<dest_node> (but not both) identify the local node, the
contact is "discovered".  A discovered contact is an opportunity to transmit
data either to or from the local node that has spontaneously occurred in
real time.  I<data_rate> must be greater than zero (the discovered data
rate).  When inserted into the contact plan, the contact's start time is
automatically changed to the current time and its stop time is automatically
changed from zero to the maximum POSIX time or to the start time of the
earliest subsequent scheduled contact in the contact plan with the same
source and destination, whichever is earlier.  When the discovered contact
ends, the discovered contact is automatically transformed into a latent
contact, simulating the addition of a latent contact as discussed above:
its I<start_time>, I<stop_time>, I<confidence>, I<data_rate> are all set
to zero.

=item 4  scheduled

When I<stop_time> is greater than zero and I<confidence> is 1.0, the contact
is "scheduled".  A scheduled contact is a managed opportunity to transmit data
between nodes, as inferred (for example) from a spacecraft or ground station
operating plan.  I<stop_time> must be greater than I<start_time> and
I<data_rate> must be greater than zero.

=item 5  predicted

When I<stop_time> is greater than zero and I<confidence> is less than 1.0,
the contact is "predicted".  A predicted contact is a statistically likely
opportunity to transmit data between nodes, as inferred from the network's
history of contact discoveries.  As for a scheduled contact, I<stop_time>
must be greater than I<start_time> and I<data_rate> must be greater than zero.
