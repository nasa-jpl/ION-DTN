...
B<ltpclo> I<remote_node_identifier>
...
protocol agent identified by I<remote_node_identifier> and passes them
to the local LTP engine for aggregation, segmentation, and transmission
to the remote node.
...
No LTP outduct with duct name matching I<remote_node_identifier> has been
added to the BP database.  Use B<bpadmin> to stop the LTP convergence-layer
...
bpadmin(1), ltpadmin(1), ltpcli(1), bprc(5), ltprc(5)
