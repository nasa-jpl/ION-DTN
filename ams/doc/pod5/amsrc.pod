...
A recent update to AMS introduced an explicit routing model to support modern
three-part IPN endpoint IDs and work around the 16-bit limitation of continuum
numbers.  This change decouples the application-layer AMS continuum number
from the transport-layer network node number.  Routing information is now
explicitly defined using a mandatory Gateway Endpoint ID (gweid) for each
message space, which maps the continuum to a specific network gateway address
(e.g., a Bundle Protocol endpoint).

AMS entities automatically load their MIBs from initialization files at
startup.  When AMS is built with the --without-expat option configured (default),
the MIB initialization file must conform to the I<amsrc> syntax described
...
must be of the form <hostname>:<port_number>.
...
by the venture that is the element that's currently being configured.  Each
msgspace definition maps a remote continuum number to a specific network
gateway address.
...
(Mandatory) Identifies the RAMS network endpoint ID of the RAMS gateway
module for this message space.  Gateway endpoint ID is expressed as
...
of the form <hostname>:<port_number>.
...
+venture nbr=1 appname=amsdemo authname=test gweid="bp@ipn:2.1"
...
*msgspace nbr=1 neighbor=1 gweid='bp@ipn:1.1'

*msgspace nbr=2 neighbor=0 gweid='bp@ipn:2.1'
