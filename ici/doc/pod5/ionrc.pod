...
will be I<distance> light seconds.  I<distance> may optionally contain up
to three fractional digits; for example, C<0.006> denotes a one-way light
time of 6 milliseconds.  Integer values retain their original whole-second
meaning.
...
=item B<m utcdelta> I<local_time_sec_after_UTC>[I<.milliseconds>]
...
computer.  The value may include one to three fractional decimal digits,
for example C<0.250> or C<-0.125>.  This delta is automatically applied to locally
...
=item B<m clockerr> I<known_maximum_clock_error>[I<.milliseconds>]

This management command sets ION's understanding of the accuracy of the
scheduled start and stop times of planned contacts, in seconds with optional
