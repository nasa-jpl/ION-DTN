...
=item B<g> I<multicast group number for TC blocks>

The B<application selection> command.  the command identifies the specific
TC application to which all subsequently submitted commands pertain.  A TC
application is uniquely identified by the group number of the Bundle Protocol
multicast group comprising all nodes hosting TC clients that subscribe
to TC "blocks" published for that application.

=item B<1> I<multicast group number for TC bulletins> I<multicast group number for TC records> I<number of authorities in collective> I<K> I<R>
...
=item B<a> I<node number>

This command adds the node identified by I<node number> to the list of nodes
hosting B<authorized clients> for the selected TC application.  Once this
...
=item B<d> I<node number>

This command deletes the node identified by I<node number> from the list of
nodes hosting B<authorized clients> for the selected TC application.

=item B<l>

This command lists all nodes currently hosting B<authorized clients> for
