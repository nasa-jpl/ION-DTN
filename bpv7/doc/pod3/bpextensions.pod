...
must both be delivered to a local client and also forwarded to another node.
...
=item void discardAcqExtensionBlock(AcqExtBlock *blk)
...
=item SdrObject findExtensionBlock(Bundle *bundle, BpBlockType type, char tag)
