...
=head1 SYNOPSIS
    ...
    typedef int (*RTBHandler)(time_t time, unsigned long count, char *buffer,
...
in the bss.h header file.  The application is not responsible for populating
this structure; it's strictly for the private use of the BSS library.

=over 4

=item int bssOpen(char *bssName, char *path)
...
the directory in which the database resides.  On detection of existing opened
database, do nothing and returns -2; on failure to open database returns -1; on
success, returns 0.
...
=item int bssRun(char *bssName, char *path, char *eid, char *buffer,
long bufLen, RTBHandler handler)
...
=item long bssSeek_read(bssNav *nav, time_t time, time_t *curTime,
...
=item long bssNext_read(bssNav *nav, time_t *curTime, unsigned long *count,
...
=item long bssPrev_read(bssNav *nav, time_t *curTime, unsigned long *count,
char *data, long dataLen)
