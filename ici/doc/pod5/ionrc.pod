...
This management command reports whether or not the local node is allowed to
serve as a passageway (bridge) for inter-regional forwarding.  A node that is
a member of two or more regions only relays bundles between those regions when
this flag is set; the default value is 0 (opt-in).  A node may originate
inter-regional bundles regardless of this flag.

If a Boolean argument is provided when the command is executed, the flag is
revised to conform with the asserted value.
