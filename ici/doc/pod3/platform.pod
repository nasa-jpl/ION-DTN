...
I<platform> is a library of functions that simplify the porting of
...
of the I<SPACE_ORDER> macro.  "Space order" is the base 2 log of the
...
useful for some applications.  For this purpose, I<platform> defines
new types B<vast> and B<uvast> (unsigned vast) which are consistently
...
netdb.h, sys/uni.h, and fcntl.h.  Beyond this, I<platform> attempts
...
=head2 PLATFORM COMPATIBILITY PATCHES
...
=head2 PLATFORM GENERIC MACROS AND FUNCTIONS
...
The FDTABLE_SIZE macro returns the total number of file
descriptors defined for the process (or VxWorks target).

=item ION_PATH_DELIMITER

The ION_PATH_DELIMITER macro returns the ASCII character -- either '/' or
'\' -- that is used as a directory name delimiter in path names for the
file system used by the local platform.

=item ION_FORMAT_PRINTF(fmt_arg, first_vararg)

Safely leverages compiler static analysis to validate variadic format strings
without breaking strict ISO C or POSIX compliance.

=over 4

=item Diagnostic Build-Time Tool:

This macro is strictly a diagnostic build-time tool; it does not interject
any runtime dependencies. When compiled with GNU or Clang toolchains, it
evaluates to C<__attribute__((format(printf, fmt_arg, first_vararg)))>,
which natively instructs the compiler frontend to validate format string
arguments during the build process (triggering C<-Wformat> warnings or
errors). The resulting generated machine code behaves identically whether
the attribute is present or absent, ensuring runtime logic and
cross-platform portability are 100% preserved.

=item Strict Portability via Preprocessor Guards:

The implementation strictly avoids reliance on undefined or unspecified
compiler behavior. It utilizes standard preprocessor conditional guards
(C<#if defined(__GNUC__) || defined(__clang__)>) to ensure cross-platform
compliance. When compiled on a strict, non-GNU ISO C compiler, the
preprocessor resolves C<ION_FORMAT_PRINTF> to an empty macro. This
guarantees the compiler processes pure, vanilla ISO C, entirely stripped
of any GNU extensions.

=item ISO C18 Compliance:

The presence of C<__attribute__> within the conditional block does not
violate the ISO C99, C11, or C17/C18 standards. Section 7.1.3 of the
ISO/IEC 9899:2018 standard explicitly dictates that "All identifiers that
begin with an underscore and either an uppercase letter or another
underscore are always reserved for any use." The C standard deliberately
reserved this double-underscore namespace so compiler vendors could
implement built-in diagnostic features without colliding with strictly
compliant user code.

=item Disabling the Macro:

For strict compliance audits that reject the presence of the literal string
C<__attribute__>, developers can completely disable the macro's evaluation.
To do so, modify F<platform.h> to unconditionally define
C<ION_FORMAT_PRINTF(fmt_arg, first_vararg)> to an empty macro, thereby
removing the diagnostic checks globally.

=back

=item oK(expression)

The oK macro simply casts the value of I<expression> to void, a way of
handling function return codes that are not meaningful in this context.

=item CHKERR(condition)

The CHKERR macro is an "assert" mechanism.  It causes the calling function
to return -1 immediately if I<condition> is false.

=item CHKZERO(condition)

The CHKZERO macro is an "assert" mechanism.  It causes the calling function
to return 0 immediately if I<condition> is false.

=item CHKNULL(condition)

The CHKNULL macro is an "assert" mechanism.  It causes the calling function
to return NULL immediately if I<condition> is false.

=item CHKVOID(condition)

The CHKVOID macro is an "assert" mechanism.  It causes the calling function
to return immediately if I<condition> is false.

=item void snooze(unsigned int seconds)

Suspends execution of the invoking task or process for the indicated
number of seconds.

=item void microsnooze(unsigned int microseconds)

Suspends execution of the invoking task or process for
the indicated number of microseconds.

=item void getCurrentTime(struct timeval *time)

Returns the current local time (ctime, i.e., Unix epoch time) in a timeval
structure (see gettimeofday(3C)).

=item void isprintf(char *buffer, int bufSize, char *format, ...)
...
it always NULL-terminates the string in I<buffer>, even if the length of the
composed string would equal or exceed I<bufSize>.  Buffer overruns are
reported by log message; unlike snprintf(), isprintf() returns void.

=item size_t istrlen(const char *sourceString, size_t maxlen)
...
argument, the maximum valid length of I<sourceString>.  The function
returns the number of non-NULL characters in I<sourceString> preceding
the first NULL character in I<sourceString>, provided that a NULL
character appears somewhere within the first I<maxlen> characters of
I<sourceString>; otherwise it returns I<maxlen>.

=item char *istrcpy(char *buffer, const char *sourceString, size_t bufSize)
...
third argument, the total size of the buffer into which I<sourceString>
is to be copied.  istrcpy() always NULL-terminates the string in I<buffer>,
even if the length of I<sourceString> string would equal or exceed
I<bufSize> (in which case I<sourceString> is truncated to fit within
the buffer).

=item char *istrcat(char *buffer, char *sourceString, size_t bufSize)
...
aggregated.  istrcat() always NULL-terminates the string in I<buffer>, even
if the length of I<sourceString> string would equal or exceed the sum of
I<bufSize> and the length of the string currently occupying the buffer
(in which case I<sourceString> is truncated to fit within the buffer).

=item char *igetcwd(char *buf, size_t size)
...
=item void isignal(int signbr, void (*handler)(int))
...
=item int ifopen(const char *fileName, int flags, int pmode)
...
I<fileName> does not identify a regular file, i.e., it's a directory, a
named pipe, etc.

B<NOTE> that ION also provides iopen() which is nothing more than a
portable wrapper for open().  iopen() can be used to open a directory, for
example.

=item char *igets(int fd, char *buffer, int buflen, int *lineLen)

igets() reads a line of text, delimited by a newline character, from I<fd>
into I<buffer> and writes a NULL character at the end of the string.  The
newline character itself is omitted from the NULL-terminated text line in
I<buffer>; if the newline is immediately preceded by a carriage return
...
I<buffer>.  End of file is interpreted as an implicit newline, terminating
the line.  If the number of characters preceding the newline is greater
than or equal to I<buflen>, only the first (I<buflen> - 1) characters of
the line are written into I<buffer>.  On error the function sets I<*lineLen>
to -1 and returns NULL.  On reading end-of-file, the function sets I<*lineLen>
to zero and returns NULL.  Otherwise the function sets I<*lineLen> to the
length of the text line in I<buffer>, as if from strlen(3), and returns
I<buffer>.

=item int iputs(int fd, char *string)

iputs() writes to I<fd> the NULL-terminated character string at I<string>.  No
terminating newline character is appended to I<string> by iputs().  On error
the function returns -1; otherwise the function returns the length of the
character string written to I<fd>, as if from strlen(3).

=item vast strtovast(char *string)

Converts the leading characters of I<string>, skipping leading white space
and ending at the first subsequent character that can't be interpreted as
contributing to a numeric value, to a B<vast> integer and returns that integer.

=item uvast strtouvast(char *string)

Same as strtovast() except the result is an unsigned B<vast> integer value.

=item int platform_parse_uvast(const char *string, uvast *result)

Parses a string into an unsigned B<vast> integer with strict POSIX validation.
Unlike standard string conversion functions, this utility protects against
two's complement negative wrapping by explicitly rejecting negative inputs,
prevents LP64/ILP32 truncation vulnerabilities by respecting architecture-specific
bounds, and catches trailing garbage characters. If the string is successfully
parsed and passes all boundary checks, the resulting integer is stored in
I<*result> and the function returns 0. On any parsing failure, out-of-bounds
error, or trailing garbage detection, the function returns -1.

=item int platform_parse_int(const char *string, int *result)

Parses a string into a standard signed C integer with strict validation.
Similar to platform_parse_uvast(), this function strictly enforces the
architectural limits of the integer data type (INT_MAX and INT_MIN) to
prevent overflow and underflow vulnerabilities while aggressively catching
trailing garbage characters. Returns 0 on successful parsing and assignment
to I<*result>, and returns -1 on any failure.

=item int platform_parse_double(const char *str, double *result)

Safely parses a string into a double-precision floating-point number.
This function provides strict validation by utilizing strtod() while
actively catching overflow and underflow conditions via ERANGE. It
explicitly rejects NaN (Not-a-Number) and Infinity values, making it
safe for command-line inputs where real numeric values are required.
It also validates the entire string by checking for and rejecting any
invalid trailing garbage characters. Returns 0 on successful parsing
and assignment to I<*result>, and returns -1 on any failure.

=item void findToken(char **cursorPtr, char **token)

Locates the next non-whitespace lexical token in a character array, starting
at I<*cursorPtr>.  The function NULL-terminates that token within the array
and places a pointer to the token in I<*token>.  Also accommodates tokens
enclosed within matching single quotes, which may contain embedded spaces
and escaped single-quote characters.  If no token is found, I<*token> contains
NULL on return from this function.

=item void *acquireSystemMemory(size_t size)

Uses memalign() to allocate a block of system memory of length I<size>,
...
=item int createFile(const char *name, int flags)
...
=item unsigned int getInternetAddress(char *hostName)

Returns the IP address of the indicated host machine, or zero if the
address cannot be determined.

=item char *getInternetHostName(unsigned int hostNbr, char *buffer)

Writes the host name of the indicated host machine into I<buffer> and
returns I<buffer>, or returns NULL on any error.  The size of I<buffer>
should be (MAXHOSTNAMELEN + 1).

=item int getNameOfHost(char *buffer, int bufferLength)

Writes the first (I<bufferLength> - 1) characters of the
host name of the local machine into I<buffer>.  Returns 0 on success, -1 on
any error.

=item unsigned int getAddressOfHost()

Returns the IP address for the host name of the local machine, or 0 on any
error.

=item void parseSocketSpec(char *socketSpec, unsigned short *portNbr, unsigned int *hostNbr)

Parses I<socketSpec>, extracting host number (IP address) and port number from
the string.  I<socketSpec> is expected to be of the form
"{ @ | hostname }[:<portnbr>]", where @ signifies "the host name of the
local machine".  If host number can be determined, writes it into I<*hostNbr>;
otherwise writes 0 into I<*hostNbr>.  If port number is supplied and
is in the range 1024 to 65535, writes it into I<*portNbr>; otherwise writes
0 into I<*portNbr>.

=item void printDottedString(unsigned int hostNbr, char *buffer)

Composes a dotted-string (xxx.xxx.xxx.xxx) representation of the IPv4 address
in I<hostNbr> and writes that string into I<buffer>.  The length of I<buffer>
must be at least 16.

=item char *getNameOfUser(char *buffer)

Writes the user name of the invoking task or process
into I<buffer> and returns I<buffer>.  The size of I<buffer>
must be at least I<L_cuserid>, a constant defined in the
stdio.h header file.  Returns I<buffer>.

=item int reUseAddress(int fd)

Makes the address that is bound to the socket identified by
I<fd> reusable, so that the socket can be closed
and immediately reopened and re-bound to the same port number.
Returns 0 on success, -1 on any error.

=item int makeIoNonBlocking(int fd)

Makes I/O on the socket identified by I<fd> non-blocking; returns -1 on
...
=item int watchSocket(int fd)

Turns on the "linger" and "keepalive" options for the
socket identified by I<fd>.  See socket(2) for details.  Returns 0 on
success, -1 on any failure.

=item void closeOnExec(int fd)

Ensures that I<fd> will NOT be open in any child process
fork()ed from the invoking process.  Has no effect on a VxWorks platform.

=back

=head2 EXCEPTION REPORTING
...
=item void writeMemo(char *msg)

Writes one log message, using the currently defined message logging function.

=item void writeMemoNote(char *msg, char *note)

Writes a log message like writeMemo(), accompanied by the user-supplied
context-specific text in I<note>.

=item void writeErrMemo(char *msg)

Writes a log message like writeMemo(), accompanied by text describing the
current system error.

=item char *itoa(int value)

Returns a string representation of the signed integer in I<value>, nominally
for immediate use as an argument to putErrmsg().

=item char *utoa(unsigned int value)

Returns a string representation of the unsigned integer in I<value>, nominally
for immediate use as an argument to putErrmsg().

=item char *vasttoa(vast value)

Returns a string representation of I<vast> in I<value>, nominally
for immediate use as an argument to putErrmsg().

=item char *uvasttoa(uvast value)

Returns a string representation of I<uvast> in I<value>, nominally
for immediate use as an argument to putErrmsg().

=item char *sizetoa(size_t value)

Returns a string representation of I<size_t> in I<value>, nominally
for immediate use as an argument to putErrmsg().

=item void postErrmsg(const char *text, const char *argument)

Constructs an error message noting the name of the source file containing
the line at which this function was called, the line number, the I<text> of
the message, and -- if not NULL -- a single textual I<argument> that can be
...
a privately managed pool of static memory, ERRMSGS_BUFSIZE bytes in length.

If I<text> is NULL or is a string of zero length or begins with a newline
character (i.e., I<*text> == '\0' or '\n'), the function returns immediately
...
=item void postSysErrmsg(const char *text, const char *arg)

Like postErrmsg() except that the error message constructed by the function
additionally contains text describing the current system error.  I<text> is
...
=item int getErrmsg(char *buffer)

Copies the oldest error message in the message pool into I<buffer> and
...
into I<buffer>.  Note that, for safety, the size of I<buffer> should be
ERRMSGS_BUFSIZE.
...
=item void writeErrmsgMemos(void)
...
Note that (a) printStackTrace() is B<only> implemented for Linux platforms
at this time; (b) symbolic names of functions can only be printed if the
I<-rdynamic> flag was enabled when the executable was linked; (c) only the
...
the stack trace snapshot was taken, use the Linux I<addr2line> tool. To do
this, cd into a directory in which the executable file resides (such as
/opt/bin) and submit an addr2line command as follows:

=over 4

addr2line -e I<name_of_executable> I<stack_frame_address>

=back

where both I<name_of_executable> and I<stack_frame_address> are taken from
...
=item void setWatcher(Watcher usersWatcherName)
...
=head2 SELF-DELIMITING NUMERIC VALUES (SDNV)
...
=item void encodeSdnv(Sdnv *sdnvBuffer, uvast value)

Determines the number of octets of SDNV text needed to contain the value,
places that number in the I<length> field of the SDNV buffer, and encodes
the value in SDNV format into the first I<length> octets of the I<text> field
of the SDNV buffer.

=item int decodeSdnv(uvast *value, unsigned char *sdnvText)

Determines the length of the SDNV located at I<sdnvText> and returns this
number after extracting the SDNV's value from those octets and storing it
in I<value>.  Returns 0 if the encoded number value will not fit into an
unsigned vast integer.

Note that decodeSdnv() is B<unbounded>: it keeps consuming octets until it
encounters an octet whose high-order bit is clear.  It therefore assumes
that I<sdnvText> points into a buffer that is known to contain a complete,
well-formed SDNV.  Do B<not> call decodeSdnv() on untrusted or
length-delimited input (for example, bytes just received from a
convergence-layer adapter): a truncated SDNV whose octets all have the
high-order bit set will drive a read past the end of the buffer.  Use
decodeSdnvBounded() in that case.

=item size_t decodeSdnvBounded(uvast *value, unsigned char *sdnvText, size_t length)

The bounds-checked counterpart to decodeSdnv(), for use whenever the SDNV
is read from untrusted or length-delimited input.  I<length> is the number
of octets remaining in the buffer at I<sdnvText>, i.e., the maximum number
of octets the function is permitted to read.  The SDNV must be fully
contained within those first I<length> octets: the function never reads at
or past I<sdnvText> + I<length>, so a truncated SDNV at the end of the
buffer cannot drive an out-of-bounds read, and the returned length never
exceeds I<length>.

On success the function stores the decoded value in I<value> and returns
the length of the SDNV in octets, a number from 1 through 10.

The function returns 0 to indicate failure, which occurs if the SDNV is
truncated within the first I<length> octets, or if the encoded value will
not fit into an unsigned vast integer (i.e., is longer than 70 bits).  In
both of these cases I<value> is set to 0.  A return value of 0 must always
be treated as a decode failure: the caller must not advance its parse
cursor by the return value and must not trust the contents of I<value>.
The function additionally returns 0 without storing anything if I<value> or
I<sdnvText> is NULL, or if I<length> is 0.

decodeSdnvBounded() holds no locks and requires no SDR transaction.  It
reads only the caller-supplied buffer and writes only through I<value>,
producing no other side effects, so it is re-entrant and thread-safe as
long as callers operate on distinct buffers.

=back

=head2 ARITHMETIC ON LARGE INTEGERS (SCALARS)
...
=item void loadScalar(Scalar *scalar, signed int value)

Sets the value of I<scalar> to the absolute value of I<value>.

=item void increaseScalar(Scalar *scalar, signed int value)

Adds to I<scalar> the absolute value of I<value>.

=item void reduceScalar(Scalar *scalar, signed int value)

Adds to I<scalar> the absolute value of I<value>.

=item void multiplyScalar(Scalar *scalar, signed int value)

Multiplies I<scalar> by the absolute value of I<value>.

=item void divideScalar(Scalar *scalar, signed int value)

Divides I<scalar> by the absolute value of I<value>.

=item void copyScalar(Scalar *to, Scalar *from)

Copies the value of I<from> into I<to>.

=item void addToScalar(Scalar *scalar, Scalar *increment)

Adds I<increment> (a Scalar rather than a C integer) to I<scalar>.

=item void subtractFromScalar(Scalar *scalar, Scalar *decrement)

Subtracts I<decrement> (a Scalar rather than a C integer) from I<scalar>.

=item int scalarIsValid(Scalar *scalar)

Returns 1 if the arithmetic performed on I<scalar> has not resulted in
overflow or underflow.

=item int scalarToSdnv(Sdnv *sdnv, Scalar *scalar)

If I<scalar> points to a valid Scalar, stores the value of I<scalar> in
I<sdnv>; otherwise sets the length of I<sdnv> to zero.

=item int sdnvToScalar(Scalar *scalar, unsigned char *sdnvText)

If I<sdnvText> points to a sequence of bytes that, when interpreted as
the text of an Sdnv, has a value that can be represented in a 61-bit
unsigned binary integer, then this function stores that value in I<scalar>
...
=item int initResourceLock(ResourceLock *lock)

Establishes an inter-thread lock for use in locking some resource.  Returns
0 if successful, -1 if not.

=item void killResourceLock(ResourceLock *lock)

Deletes the resource lock referred to by I<lock>.

=item void lockResource(ResourceLock *lock)

Checks the state of I<lock>.  If the lock is already
...
=item void unlockResource(ResourceLock *lock)

If called by the current owner of I<lock>, decrements I<lock>'s
...
=head2 SHARED MEMORY IPC DEVICES
...
I<NOTE> that this is distinct from the VxWorks "VxMP" capability enabling
...
=item void sm_ipc_stop( )

Releases shared-memory IPC management resources, disabling the shared-memory
IPC functions until sm_ipc_init() is called again.

=item int sm_GetUniqueKey( )

Some of the "sm_" (shared memory) functions described
below associate new communication objects with I<key>
...
=item sm_SemId sm_SemCreate(int key, int semType)
...
or SM_SEM_NONE on any failure.  If I<key> refers to an existing
semaphore, returns the handle of that semaphore.  If
I<key> is the constant value SM_NO_KEY, automatically
obtains an unused key.  On VxWorks platforms, I<semType>
...
supported and I<semType> is ignored.

=item int sm_SemTake(sm_SemId semId)
...
=item int sm_SemEnded(sm_SemId semId)
...
=item void sm_SemUnend(sm_SemId semId)
...
=item int sm_SemUnwedge(sm_SemId semId, int timeoutSeconds)
...
I<timeoutSeconds> seconds (providing time for normal processing to be
...
=item void sm_SemDelete(sm_SemId semId)

Destroys the indicated semaphore.

=item sm_SemId sm_GetTaskSemaphore(int taskId)

Returns the ID of the semaphore that is dedicated to the private use of the
indicated task, or SM_SEM_NONE on any error.
...
=item int sm_ShmAttach(int key, int size, char **shmPtr, int *id)
...
shared memory segment identified by I<key>.  If I<key> is the constant value
SM_NO_KEY, automatically sets I<key> to some unused key value.
If a shared memory segment identified by I<key> already exists, then
I<size> may be zero and the value of I<*shmPtr> is ignored.
Otherwise the size of the shared memory segment must be provided
in I<size> and a new shared memory segment is created in a manner that is
dependent on I<*shmPtr>: if I<*shmPtr> is NULL then
I<size> bytes of shared memory are dynamically acquired, allocated, and
assigned to the newly created shared memory segment; otherwise the
memory located at I<shmPtr> is assumed to have been pre-allocated
and is merely assigned to the newly created shared memory segment.

On success, stores the unique shared memory ID of the segment in I<*id>
for possible future destruction, stores a pointer to the segment's
assigned memory in I<*shmPtr>, and returns 1 (if the segment is newly
created) or 0 (otherwise).  Returns -1 on any error.

=item void sm_ShmDetach(char *shmPtr)

Unregisters the invoking task or process as a user of
the shared memory starting at I<shmPtr>.

=item void sm_ShmDestroy(int id)

Destroys the shared memory segment identified by I<id>, releasing any
memory that was allocated when the segment was created.

=back

=head2 PORTABLE MULTI-TASKING

=over 4

=item int sm_TaskIdSelf( )

Returns the unique identifying number of the invoking task or process.

=item int sm_TaskExists(int taskId)

Returns non-zero if a task or process identified by
I<taskId> is currently running on the local processor, zero otherwise.

=item void *sm_TaskVar(void **arg)
...
I<arg> is non-NULL, then I<*arg> is posted as the new value of the task's
private task variable.  In any case, the value of that task variable is
returned.

=item void sm_TaskSuspend( )
...
=item void sm_TaskDelay(int seconds)

Same as snooze().

=item void sm_TaskYield( )

Relinquishes CPU temporarily for use by other tasks.

=item int sm_TaskSpawn(char *name, char *arg1, char *arg2, char
...
command-line arguments.  I<name> is the name of the
function (VxWorks) or executable image (UNIX) to be executed
in the new task/process.

For UNIX, I<name> must be the name of some executable
program in the $PATH of the invoking process.

For VxWorks, I<name> must be
...
=item void sm_TaskKill(int taskId, int sigNbr)

Sends the indicated signal to the indicated task or process.

=item void sm_TaskDelete(int taskId)
...
=item int pseudoshell(char *script)

Parses I<script> into a command name and up to 10 arguments, then passes the
...
I<script> are normally whitespace-delimited, but a token that is enclosed in
single-quote characters (') may contain embedded whitespace and may contain
escaped single-quote characters ("\'").  On any parsing
failure returns -1; otherwise returns the value returned by sm_TaskSpawn().
