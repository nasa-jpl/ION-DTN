...
Dependency-free (stdlib unittest only), like the checker itself:
    ...
    python3 tools/test_check_doc_consistency.py
"""
...
class ManageSubcommandsTest(unittest.TestCase):
    def test_only_inside_execute_manage(self):
        ...
        """
        self.assertEqual(cdc.manage_subcommands_in_src(src),
                         {"heapmax", "usage"})
...
class HeaderFunctionsTest(unittest.TestCase):
    def test_prototypes_macros_and_comments(self):
        ...
        self.assertEqual(names, {"sdr_malloc", "sdr_begin_xn", "sdr_exit_xn",
                                 "sdr_string"})
...
class NamespacePrefixesTest(unittest.TestCase):
    def test_underscore_and_camel_case(self):
        self.assertEqual(cdc.namespace_prefixes({"bp_send", "bp_receive"}),
                         {"bp_"})
        self.assertEqual(cdc.namespace_prefixes({"ionAttach", "ionDetach"}),
                         {"ion"})
...
=item void bp_detach()
"""
        self.assertEqual(cdc.pod_api_functions(pod),
                         {"bp_send", "bssRun", "bp_detach"})
...
=item bug item
"""
        self.assertEqual(cdc.pod_diagnostics(pod),
...
class AdminConfigFileRefsTest(unittest.TestCase):
    def test_named_file_in_dos2unix_boilerplate(self):
        pod = ("If you edit the B<ionrc> file on a Windows machine, be sure to "
               "use dos2unix ...")
...
class DiagnosticMatchingTest(unittest.TestCase):
    ...
    def test_program_prefix_and_placeholders_ignored(self):
        diag = cdc._diag_tokens("lgsend: can't open own endpoint I<eid>.",
                                "lgsend")
        self.assertEqual(diag, {"cant", "open", "own", "endpoint"})
        self.assertTrue(cdc.diagnostic_is_documented(
    ...
    def test_stale_message_is_flagged(self):
        ...
        self.assertFalse(cdc.diagnostic_is_documented(
...
class FindingBuilderTest(unittest.TestCase):
    ...
    def test_manage_subcommand_findings(self):
        pod = "=item B<m usage>\n"
        src = ('void executeManage(int c, char **tokens) '
    ...
    def test_api_findings_limit_reverse_direction_to_namespace(self):
        findings = cdc.api_findings("bp.pod", {"bp_send"},
                                    {"bp_send", "bp_receive", "MAXLEN",
                                     "unrelatedHelper"})
        self.assertEqual(
            findings,
    ...
    def test_admin_rc_findings(self):
        self.assertEqual(cdc.admin_rc_findings("p/ionsecadmin.pod",
    ...
    def test_diagnostics_findings(self):
        ...
        self.assertEqual(
            cdc.diagnostics_findings("p/bpclock.pod", "bpclock",
                                     ["Can't dispatch events.",
                                      "Can't reach the moon."], src_tokens),
...
class CollectFindingsTest(unittest.TestCase):
    def setUp(self):
        ...
        for rel, text in (("foo/doc/pod5/foorc.pod", POD5),
    ...
    def test_missing_files_are_reported(self):
        ...
        self.assertIn("[admin] MISSING FILE: foo/doc/pod5/gone.pod or "
                      "foo/utils/gone.c",
    ...
    def test_missing_synopsis_is_reported(self):
        ...
        self.assertIn("[pod1] foo/doc/pod1/fooadmin.pod: missing "
                      "'=head1 SYNOPSIS' section",
