...
=item B<m port> I<port number>

Port number.  This command sets the port number of the sockets on which
the node will announce and listen for beacon messages.
...
Broadcast interval.  This command sets the beacon messages period on
...
addresses will listen for broadcast packets.
