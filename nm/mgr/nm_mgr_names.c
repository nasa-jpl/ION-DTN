...
int names_add_name(char *name, char *desc, int adm, char *mid_str)
{
	...
	if((entry = (mgr_name_t *) MTAKE(sizeof(mgr_name_t))) == NULL)
	{
		return 0;
	}
...
mid_t *names_get_mid(int adm_type, int mid_type, int mid_cat, int idx)
{
	LystElt elt;
	mgr_name_t *cur = NULL;
	int i = 0;
	...
	for(elt = lyst_first(gMgrNames); elt != NULL; elt = lyst_next(elt))
	{
		...
		if(((adm_type == cur->adm) || (adm_type == ADM_ALL)) &&
		   (mid_type == MID_GET_FLAG_TYPE(cur->mid->flags)) &&
		   (mid_cat == MID_GET_FLAG_CAT(cur->mid->flags)))
		{
			if(i == idx)
			{
				return cur->mid;
...
char *names_get_name(mid_t *mid)
{
	...
	for(elt = lyst_first(gMgrNames); elt != NULL; elt = lyst_next(elt))
	{
		...
		{
			if((result = (char *) MTAKE(strlen(cur->name) + 1)) == NULL)
			{
				return NULL;
			}
 ...
 * Do not remove items...
 */
Lyst names_retrieve(int adm_type, int mid_type, int mid_cat)
{
	Lyst result = NULL;
	LystElt elt;
	mgr_name_t *cur = NULL;
