...
=item B<1> I<node> [ { I<ion_config_filename> | '.' | '' } ]
...
The command configures the local node to be identified by I<node>, an
ipn-scheme node identifier which uniquely identifies the node in the
delay-tolerant network.  It also configures ION's data space (SDR)
and shared working-memory partition.  For this purpose it uses a set
of default settings if no argument follows I<node> or if
the argument following I<node> is ''; otherwise it uses
the configuration settings found in a configuration file.  If
configuration file name '.' is provided, then the configuration
...
node number 19 (allocator number zero) and using default values to configure
the data space and shared working-memory partition.
...
A B<region> is an arbitrarily managed set of nodes that customarily are able
...
This command prints the current memory protection thresholds (as set by
B<m memprotect>) along with the current free-space percentages for both the
SDR heap and working memory.  Each pool is reported as either "OK" or
...
B<NOTE> (this is really important): the enhanced ipn endpoint ID scheme
is supported in the range commands.  This may result in behavior that is
not necessarily intuitive to anyone who is used to the original ipn scheme:
the node identifier "1.1" is numerically much greater than the node identifier
"638", so the range from node 1.1 to node 638 says absolutely nothing about
the range from 638 to 1.1.
...
This command lists all predicted periods of known range in light seconds.
...
file's name will be "ranges.ionrc".  This file can be used to initialize
the contact plan for a new node.
...
=item B<m memprotect> I<heap_percent> I<wm_percent>

This management command sets the memory protection thresholds for the ION
node.  Each threshold specifies the minimum percentage of free space (0-50)
that must be maintained in the corresponding memory pool.  When free space
drops below a threshold, ION will reject new bundles to protect against
memory exhaustion.  Values are clamped to the range 0-50; setting a
threshold to 0 disables protection for that pool.  The default value for
both thresholds is 10.

For example:

	m memprotect 15 20

sets the SDR heap protection threshold to 15% free and the working memory
protection threshold to 20% free.

=item B<m routing-strategy> { B<shortest-pbat> | B<fair-routing> }

Selects the route-selection strategy used by the CGR-based forwarder
(I<ipnfw>).  The default B<shortest-pbat> picks the single route with the
earliest projected bundle arrival time (PBAT).
B<fair-routing> instead admits all routes whose terminus arrival
time is within B<m allow-routing-delay> of the best PBAT and stripes
consecutive bundles across them via smooth weighted round-robin,
weighted by each route's path bottleneck transmission rate
(min over all hops).

=item B<m allow-routing-delay> I<msec>

Sets the eligibility window for B<fair-routing>, in milliseconds.  A route
is eligible iff its terminus-arrival ETA is within I<msec> of the best
route's ETA.  Default is 1000 ms.  The current PBAT timer resolution is one
second, so values smaller than 1000 round down to a strict-tie filter.
Has only effect under B<m routing-strategy fair-routing>.
...
The startup-test command.  Reports whether the ION system is started and sets
