...
#if 0
typedef struct tskvar_s
{
	int		taskId;
	void		*value;
	struct tskvar_s	*next;
} TaskVar;

#ifndef ION_MAX_TASKS
#define	ION_MAX_TASKS		100
#endif

void	*sm_TaskVar(void *value)
{
	static TaskVar		vars[ION_MAX_TASKS];
	static TaskVar		*first = NULL;
	static ResourceLock	varsLock;
	static int		varsLockInit = 0;
	int			taskId;
	TaskVar			*var;
	TaskVar			*prevVar = NULL;
	int			i;

	if (!varsLockInit)
	{
		memset((char *) &varsLock, 0, sizeof(ResourceLock));
		if (initResourceLock(&varsLock) < 0)
		{
			return NULL;
		}

		varsLockInit = 1;
		lockResource(&varsLock);
	...
	taskId = sm_TaskIdSelf();
	for (var = first; var; var = var->next)
	{
		if (var->taskId < taskId)
		{
			prevVar = var;
			continue;
		}
		...
		if (var->taskId == taskId)
		{
			/*	Found matching task var.		*/

			if (value == ((void *) -1))	/*	Delete.	*/
			{
				if (prevVar)
				{
					prevVar->next = var->next;
				}
				else
				{
					first = var->next;
			...
			{
				if (value != NULL)	/*	Revise.	*/
				{
					var->value = value;
				}
			}

			unlockResource(&varsLock);
			return (var ? var->value : NULL);
		}

		/*	No task variable for this task.			*/

		break;
	}

	if (value == NULL			/*	Return value.	*/
	|| value == ((void *) -1))		/*	Delete task.	*/
	{
		unlockResource(&varsLock);
		return NULL;
	}

	/*	Creating new task variable in first unused table slot.	*/

	for (i = 0; i < ION_MAX_TASKS; i++)
	{
		if (vars[i].value == NULL)	/*	Available.	*/
		{
			break;
		}
	}

	if (i == ION_MAX_TASKS)			/*	Too many tasks.	*/
	{
		unlockResource(&varsLock);
		return NULL;
	}

	/*	Found table slot in which to store this task variable.	*/

	vars[i].taskId = taskId;
	vars[i].value = value;
	vars[i].next = var;
	var = vars + i;
	if (prevVar)
