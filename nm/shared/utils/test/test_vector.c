...
#if 0
// mocked modules
// - blob functions - stubbed here because we are not currently able to compile blob.c
blob_t*   blob_create(uint8_t *value, size_t length, size_t alloc) { return NULL; }
blob_t*   blob_copy_ptr(blob_t *src) { return NULL; }
int       blob_compare(blob_t* v1, blob_t *v2) { return 0; }
void      blob_release(blob_t *blob, int destroy) { return; }
#endif
