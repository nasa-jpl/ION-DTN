...
=item int bp_open_source(char *eid, BpSAP *ionsapPtr, int detain)
...
=item int bp_send(BpSAP sap, char *destEid, char *reportToEid, int lifespan,
int classOfService, BpCustodySwitch custodySwitch, unsigned char srrFlags,
...
requested for this bundle: BP_RECEIVED_RPT, BP_FORWARDED_RPT, BP_DELIVERED_RPT,
BP_DELETED_RPT.
...
I<ancillaryData>, if not NULL, is used to populate the Quality of Service
(QoS) extension block for this bundle. This is an ION-specific, non-standard
extension (block type 254) patterned after the IETF draft-burleigh-dtn-ecos-00
Extended Class of Service (ECOS) specification. ION's wire format differs from
the IETF draft and will not interoperate with other implementations.

The block's I<ordinal> value is used to provide fine-grained ordering within
"expedited" traffic: ordinal values from 0 (the default) to 254 (used to
designate the most urgent traffic) are valid, with 255 reserved for custody
signals. The ordinal is encoded as a full 8-bit unsigned integer in the wire
format, not as the 6-bit field specified in the IETF draft.

Note that the insertion
of application-specific extension blocks into the bundle, in addition to the
canonical extension blocks inserted automatically per the B<bpextensions.c>
file, may be requested by listing those blocks in the I<extensions> variable
of the I<ancillaryData> block.  The value of the block's I<flags> is the
...
error.  If 0 is returned, then the bundle was not accepted -- either due to
an invalid argument or because ION's memory protection thresholds have been
breached (see ionrc(5), B<m memprotect>).  In either case, a diagnostic
message will have been written to the log file.  B<IMPORTANT:> when 0 is
returned, the ADU ZCO is not consumed by bp_send() and remains the caller's
responsibility; the caller should destroy it (via zco_destroy()) or retain
...
=item int bp_track(SdrObject bundle, SdrObject trackingElt)
...
=item void bp_untrack(SdrObject bundle, SdrObject trackingElt)
...
=item int bp_suspend(SdrObject bundle)
...
=item int bp_resume(SdrObject bundle)
...
=item int bp_cancel(SdrObject bundle)
...
=item int bp_release(SdrObject bundle)
...
bpadmin(1), lgagent(1), lgsend(1), bpextensions(3), bprc(5), lgfile(5)
