...
the directory in which the database resides.  I<eid> is ignored.
...
buffer; received bundle payloads in excess of this length will be truncated
to fit into the buffer.  I<handler> identifies the display function to which
each in-order bundle payload will be passed.

On any failure, returns -1.  On success, returns zero.

=item int bssRun(char *bssName, char *path, char *eid, char *buffer, int bufLen, RTBHandler handler)

A convenience function that performs both bssOpen() and bssStart().
...
=item void bssRead(bssNav nav, char *data, int dataLen)
...
then it will be truncated to fit into I<data>.  Note that bssRead() cannot be
successfully called until I<nav> has been populated, nominally by a preceding
call to bssSeek(), bssNext(), or bssPrev().

=item void bssSeek(bssNav *nav, time_t time, time_t *curTime, unsigned long *count)
...
bundle ID count of that bundle in I<curTime> and I<count>.

=item void bssSeek_read(bssNav *nav, time_t time, time_t *curTime, unsigned long *count, char *data, int dataLen)

A convenience function that performs bssSeek() followed by an immediate
bssRead() to return the data at the new playback position.

=item void bssNext(bssNav *nav, time_t *curTime, unsigned long *count)
...
=item void bssNext_read(bssNav *nav, time_t *curTime, unsigned long *count, char *data, int dataLen)

A convenience function that performs bssNext() followed by an immediate
bssRead() to return the data at the new playback position.

=item void bssPrev(bssNav *nav, time_t *curTime, unsigned long *count)
...
=item void bssPrev_read(bssNav *nav, time_t *curTime, unsigned long *count, char *data, int dataLen)
