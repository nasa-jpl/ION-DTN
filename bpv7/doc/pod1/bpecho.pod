...
B<bpecho> I<ownEndpointId> [I<ttl>]

=head1 DESCRIPTION

B<bpecho> uses Bundle Protocol to receive application data units
from a remote B<bpdriver> or B<bping> application task.  In response to each
received application data unit it sends back an "echo" application data
unit containing the same payload.

I<ttl> specifies the lifetime of the echo bundles in seconds.  The default
...
bpadmin(1), bpdriver(1), bpcounter(1), bping(1), bp(3)
